...
| **H-8** | **The whipsaw gate still blocks H**, which is arguably backwards for a long-gamma strategy: it skips when the intraday range exceeds 1.75× the expected move, and a wide range is the day H *wants*. Kept for the first observation window so the gating matches the rest of the fleet and the data stays comparable. **First knob to revisit once there is data.** |
