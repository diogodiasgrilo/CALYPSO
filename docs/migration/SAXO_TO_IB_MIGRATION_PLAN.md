...
| **`_snapshot_with_preflight` warmup polling** | ✅ | (this commit, 2026-05-18) | After the IBKR-required preflight, poll `live_marketdata_snapshot` up to `_SNAPSHOT_MAX_WARMUP_POLLS=8` times with `_SNAPSHOT_POLL_INTERVAL_S=0.25s` between attempts, exiting on first response with a populated field (not just `{conid, conidEx, _updated}` metadata). New `_snapshot_has_data()` helper. Total worst-case warmup: 2s. Fixes `test_get_quote_spx_index` + `test_get_vix_price_returns_float` failing on first call even during active market hours. +3 regression tests + 1 test rename in `tests/test_ib_client_reads.py`. **Superseded by a later commit (P7-audit H10): 2s proved too thin, bumped to `_SNAPSHOT_MAX_WARMUP_POLLS=12` / `_SNAPSHOT_POLL_INTERVAL_S=0.5s` (6s worst-case) — the values in this row are what shipped on 2026-05-18, not current. See `shared/ib_client.py`'s H10 comment for the current values. (Flagged as doc-drift in `PREFLIGHT_AUDIT_FINDINGS.md` finding #45; annotated here 2026-07-24 rather than rewritten, to keep this changelog row's historical record intact.)** |
