...
- **Full primary move:** ✅ **YES — implemented 2026-07-22** (changed from the earlier trade-only choice). The dashboard's CANONICAL views (main page, WebSocket, iOS widget, legacy `/api/hydra/*`) now follow the LIVE seat dynamically via `variant_readers.live_seat_id()` (reads config `dry_run`): the request endpoints follow per-request (no restart), and the WS broadcaster follows at the dashboard restart that `flip_bc_swap.sh` STEP 3 now performs. So nothing on the dashboard shows a dry variant as "the bot" once B is live. `is_live`/`is_primary` are also dynamic. +8 tests.
...
| C5 | Dashboard PRIMARY ✅ DONE — FULL MOVE (`042066e` + 2026-07-22) | MED | done | `PRIMARY_ID`→`_primary_id()`, `is_live`→`_is_live()`, AND the canonical readers (main page / WS / widget / legacy `/api/hydra/*`) all follow the live seat via `variant_readers.live_seat_id()`. Request endpoints follow per-request; the WS broadcaster follows at the dashboard restart `flip_bc_swap.sh` STEP 3 does. No hardcoded `variant_c` left on the canonical path. |
