...
> - **⚠️ Open follow-up (P5a) — breaker/warmup ALERTING is silenced.** `BrokerClient.circuit_breakers` is empty (the real breakers live in the broker now), so the strategy-side breaker/warmup alert poll is a no-op. The breakers still PROTECT the order/market/session paths; only the Telegram trip/warmup ALERTS don't fire. Fix: run `IBKRAlertHooks` inside `calypso-broker`. (`ensure_connected`-failure alerts still fire.)
...
> - **Now = Gate 2 paper-smoke watch:** observe A/B/C + the broker through the session + overnight + tomorrow's morning re-auth gate + entry windows before flipping to live-paper or merging. Then close P5a (broker-side alerting).
