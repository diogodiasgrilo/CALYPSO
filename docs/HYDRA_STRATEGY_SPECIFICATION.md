...
**Conditional entries** only fire when MKT-035 triggers (SPX drops >= 0.3% below session high). They are always call-only. On non-down days, they are silently skipped before any strike calculation.
...
```
18.5 MKT-035: Check if SPX < session_high × (1 - threshold)
...
| MKT-035 | Call-Only on Down Days | v1.11.0 | When SPX drops >= 0.3% below session high, convert to call-only (no puts). Stop = call_credit + $250 theo put + buffer. Conditional entries (12:45, 13:15) only fire on down days. 20-day data: 71% put stop rate on down days vs 7% call stop rate |
...
| 12:45, 13:15 | Conditional E6/E7 (MKT-035 only — fires as call-only when SPX drops >= 0.3% below session high) |
...
| `downday_threshold_pct` | `0.003` | MKT-035: SPX must drop this % below session high to trigger (0.3%) |
