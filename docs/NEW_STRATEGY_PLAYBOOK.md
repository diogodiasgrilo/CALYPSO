...
| 2026-09-23 | 0DTE Long Strangle (Tompkins / Theta Profits) — variant `h` (`long_strangle`) | **Step 0 only** (classify + spec) | **MEDIUM build — awaiting operator go/no-go for Step 1** | Spec: [`LONG_STRANGLE_STRATEGY_SPECIFICATION.md`](LONG_STRANGLE_STRATEGY_SPECIFICATION.md). **Step 0 changed the estimate**: a verbal "light, mostly G with the sign flipped" did not survive reading the base. The strategy is **single-day AND net-debit**, a combination neither row of Step 0's table covers. Lighter than D/E (single expiry, single day → **skip Step 6** entirely, no sidecar, no multi-day settlement); heavier than F/G because `IronCondorEntry`'s P&L is credit-shaped in every branch (`credit − value`; `call_side_expired` = *"PROFIT — kept credit"*, which **inverts** for a long strangle) and `trade_entries` has no debit columns — storing a debit as negative credit would pollute shared analyzers and a migration would touch the table B trades on live. → **Step 2 (small model) and Step 7 (isolated DB, D/E precedent) are IN.** Needs a **new `GroupMeta`**: all three existing groups are premium-selling. Genuinely new work: debit P&L, **IV percentile (nothing computes it today)**, percent-of-debit exits, "sizing for zero". Notably **simpler** than every existing variant on risk: no stop machinery (max loss = debit) and naked-short exposure is structurally impossible — a partial fill leaves a *long* option, so the failure that cost B −$159.50 on 2026-09-22 cannot occur. Claims (80% WR, +50–100%) are single-creator, unaudited, and imply ~+40% expected per trade — **build to measure, not because they are believed.** | Claude |
| 2026-09-23 | 0DTE Long Strangle — variant `h` (`long_strangle`) | **Steps 1–10 build** (`caf40c9`, `c5f164f`, `c3195bc`, `5cad58a`, `a09c526`, `d7b23e4`, `9d7afdc`, `90c21f4`, `175058a`, this one) | **BUILT (dry-run-LOCKED, never run)** | Step 0's MEDIUM estimate held. **Three playbook-relevant findings, all from the debit shape:** (1) **Step 5's breach-persistence instruction had to be INVERTED.** It exists so a stale tick cannot fire a false *stop* — a stop triggers on an ADVERSE spike, so waiting protects you. A long strategy's *profit target* triggers on a FAVOURABLE spike, and those revert; waiting systematically gives back the move. The sign of the position inverts the sign of the guard, and the replacement guard is quote quality, not elapsed time. **The playbook should say so for any future debit strategy.** (2) **Two inherited exits fail SILENTLY for a debit structure**: the base books a worthless expiry as "the credit kept" (→ a total loss recorded as **break-even**), and the DATA-004 sanity guard rejects every tick as "partial zero prices" because the shorts are absent by construction. Those are G's S-HIGH-2 and S-CRIT-1 mirrored — **the `short_*`-as-"is there a position" assumption is the recurring trap** for anything that populates `long_*` instead. (3) The buying-power floor had to be overridden **downward** (long options are fully paid; the base's width-derived $6-7.5k/contract would have skipped every entry and looked like "no signal") — the opposite direction to G's S2, so the playbook's "override the BP floor if it differs" is right but under-specifies which way. (4) **"Single-day → skip Step 6" is right and nearly hid a real bug.** The shared state file serialises only credit-shaped fields and its restore path **hardcodes `HydraIronCondorEntry`**, so an ordinary mid-day restart hands a debit strategy back entries with **no cost basis** — silently disabling its exit *and* crashing the settlement sweep for **every** entry that day. Fixed by reading the debit back out of the strategy's **own Step-7 database** (no sidecar, no shared-schema change). **Any future debit strategy that skips Step 6 must still answer "what does a mid-day restart do to my cost basis?"** — found by audit, not by a failing test. (5) **Step 8's "its own view if the shape is incompatible" was the right call and the bar is lower than it reads.** It is not about layout — EVERY renderer in this system assumes premium was COLLECTED, so "expired worthless" is the best outcome there and the WORST one for a debit strategy. A row in the existing comparison would have stated H's numbers backwards, not merely rendered them oddly. Two reusable details: the read-only assertion must run over **executable string literals via the AST**, because a grep also hits the docstring that names the forbidden verbs in order to explain their absence; and **`available: false` is the expected production response** for any dry-run-locked variant that is not installed, so the page must render it rather than 500. (6) **Step 9's shared-broker warning is the most transferable thing in this playbook, and it is easy to under-read.** H's entry path cost **13 broker round-trips** — 1 strike-grid fetch, 4 `_get_option_uic` calls (each internally a chain fetch PLUS a qualify) and 4 single-leg quotes — every one of them against the ONE `calypso-broker` session that the LIVE seat trades through. A dry-run variant can degrade a live one. The fix is mechanical and should be the default from Step 4 onward: **`_read_option_chain` returns BOTH rights' maps from one read, and quotes batch** — so resolve PAIRS, not legs (13 → 7, and 4 on a source that needs no chain). (7) **The snap can defeat a guard that reads as sufficient.** `select_strangle_strikes` refuses a non-positive expected move because "spot ± 0" is a straddle; it does not catch both legs ROUNDING onto the same strike, which happens whenever the move is under half the grid spacing. Any strategy that snaps two strikes derived from one quantity needs the post-snap check too. (8) **Step 10's three artifacts are not always all writable, and saying so beats writing placeholders.** H's audit returned **NO-GO** (`H_GOLIVE_SCOPE_AND_AUDIT.md`) on a finding different in kind from D's: D had a strategy whose selling point was a mid-pricing artifact; H had **zero observations** — verified on the VM, not assumed. With no measured behaviour there is nothing to strip a mechanic from, no fill data to phase and no basis for a first live size, so the **MVL plan and runbook were deferred with explicit preconditions** rather than written as fiction that would imply readiness. The generalisable rule: **for a strategy that has never run, the Step 10 output is the audit plus "install it in dry-run", not a flip plan.** Two reusable pieces: the credit-shaped readiness checklist asks questions a debit strategy cannot answer (credit received, spread width, stop level), so H got its own **HG-1..HG-10**; and the gate should name the *actual deliverable* rather than generic profitability — for H that is HG-4, its daily P&L correlated against the live seat's, where a NEGATIVE correlation justifies the strategy and a POSITIVE one is a valid and useful NO. | Claude |
| | | | | | |
