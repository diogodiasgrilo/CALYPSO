...
|---|---|---|
| **2026-09-21** (Mon) | **≈−$2,900 PROVISIONAL** · 4e/3s | **Worst live-era session per contract, and it breached the halt criteria drafted nine days earlier.** A clean up-trend day (SPX 7692.02 at 09:32 → 7779.04 at 15:39, VIX 14.6–15.1) and **every stop was call-side**. Realized call-side −$3,190 (−$455.71/contract) before the four surviving put sides settle worthless (≈+$295), so the day lands near **−$414/contract vs H1's −$400 threshold**, and 3 stop-losses vs **H3's ≥3**. H1 was calibrated at "1.6× the worst of 25 paper sessions (−$256)" — that worst was 2026-07-24, and today is **1.6–1.8× it**. ✅ **A2 %-of-width stop confirmed live**: `trigger_level = 1400.0` on all four = `0.40 × 5 × 100 × 7`. ✅ **Require-both-sides paid again** — but see the GEX note: the two skips it acted on were GEX put-side vetoes that were themselves **wrong**. e#7's short call at 7780 survived the 7779.04 high **by 0.96pt** and closed +$70 on the EOD flatten. Fleet clean: 0 crashes, `NRestarts=0`, 2 log errors, both the same benign 09:32 re-auth gate that self-healed in 35s before the first slot. |
