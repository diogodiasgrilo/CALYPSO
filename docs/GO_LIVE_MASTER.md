...
| **3** Test state | 🟡 | Suite **3608 passed / 16 skipped / 0 failed**. **`pip-audit` now CLEAN** — `cryptography` 48.0.0 → **50.0.0** (2026-09-12), clearing all 4 advisories; required `msal` 1.36→1.38, which lifted a `cryptography<49` ceiling. Integration paper-smoke still not run in the last 7 days. | Run the integration paper-smoke (`IBIND_INTEGRATION=paper`) — the only item left on this gate. |
