...
| **4** Paper history | 🔴 | 5 consecutive sessions with **no manual intervention** is unattainable at the current cadence. **Chaos test: still never run** — but now a guarded, repeatable script (`scripts/chaos_test.sh`, 2026-09-12) rather than an ad-hoc `kill -9`; it refuses unless outside RTH + broker healthy + account flat by quantity. Note the gate's "restart < 30s" is **unmeetable by design** (`RestartSec=30`) and has been corrected. | Run `chaos_test.sh` (one approved command), then a deliberate change freeze for the 5 clean sessions. |
