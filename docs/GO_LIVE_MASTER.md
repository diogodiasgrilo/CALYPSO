...
| **3** Test state | 🟡 | Suite **3608 passed / 16 skipped / 0 failed**. **`pip-audit -r requirements.txt` CLEAN**; the VM's *installed* set went **44 vulns/9 pkgs → 12/3**, none in the trading path. **Smoke repointed 2026-09-12** to `scripts/broker_paper_smoke.py` (broker-safe); the legacy pytest smoke would have **evicted the broker session** and now physically refuses to run. Check-only run today: safety gate + broker reads PASS, then **SKIPPED (exit 75) — market closed**. | (a) one **RTH** run of `broker_paper_smoke.py` for real Gate-3 evidence; (b) `starlette` 0.52→1.3 needs its own dashboard validation. |
