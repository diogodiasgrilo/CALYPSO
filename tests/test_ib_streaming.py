...
class FakeWsClient:
    ...
    def unsubscribe(self, channel: str, data: dict = None,
        ...
        # umd+conid → corresponding smd+conid is what we tracked subscribed
        if channel.startswith("umd+"):
            tail = channel[len("umd+"):]
            self._subscribed_channels.discard(f"smd+{tail}")
        return True
...
class TestSubscribeQuote:
    def test_sends_smd_channel(self, streaming, ws_client):
        ...
        assert ("smd+12345", {"fields": DEFAULT_QUOTE_FIELDS}) in ws_client.subscribe_calls
    ...
    def test_custom_fields(self, streaming, ws_client):
        ...
        assert ("smd+99", {"fields": ["31", "84"]}) in ws_client.subscribe_calls
    ...
    def test_resubscribe_with_new_fields_replaces(self, streaming, ws_client):
        ...
        assert any(c == "umd+12345" for c, _ in ws_client.unsubscribe_calls)
        smd_calls = [c for c, _ in ws_client.subscribe_calls if c == "smd+12345"]
...
class TestUnsubscribeAll:
    def test_clears_all_subscriptions(self, streaming, ws_client):
        ...
        for c in (1, 2, 3):
            assert any(ch == f"umd+{c}" for ch, _ in ws_client.unsubscribe_calls)
...
class TestRefreshLoop:
    def test_smd_topics_cycle_after_refresh_interval(self, ws_client):
        ...
        try:
            ...
            initial_sub_count = sum(1 for c, _ in ws_client.subscribe_calls if c == "smd+12345")
            # Wait through ~3 refresh cycles
            time.sleep(0.7)
            final_sub_count = sum(1 for c, _ in ws_client.subscribe_calls if c == "smd+12345")
            final_unsub_count = sum(1 for c, _ in ws_client.unsubscribe_calls if c == "umd+12345")
    ...
    def test_refresh_continues_on_individual_failure(self, ws_client):
        ...
        try:
            ...
            smd_1000 = [c for c, _ in ws_client.subscribe_calls if c == "smd+1000"]
