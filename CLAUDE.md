...
- **FOMC blackout:** Skips all entries on FOMC announcement days (configurable via `fomc_announcement_skip`)
...
### HYDRA Bot Details (v1.18.0 - Updated 2026-03-23)
- **Strategy:** MEIC + Trend Following Hybrid (EMA 20/40 direction filter)
- **Structure:** 5 base entries per day at :15/:45 marks starting 10:15 AM ET (v1.10.3 — matches winning period Feb 10-27). Full iron condors or put-only (MKT-011) + credit gate. VIX entry cutoff disabled (max_vix_entry=999, neither Tammy nor Sandvand use VIX cutoffs). Conditional entries: **E7 (13:15) ENABLED** as down-day call-only (`conditional_e7_enabled: true`, threshold 0.3% drop from open — MKT-035). **E6 (12:45) ENABLED** as up-day put-only (`conditional_upday_e6_enabled: true`, threshold 0.4% rise from open — Upday-035). Walk-forward backtest (719 days) Sharpe 2.733 vs 2.651 baseline. Base entries E1-E5 always attempt full ICs regardless of market direction.
...
- **Asymmetric Spread Widths (MKT-028):** Put spread floor raised to 75pt (call floor stays 60pt). Put longs cost 7× more than calls due to skew — wider put spreads push longs further OTM = cheaper. `margin = max(call, put) × $100`, so wider puts don't require wider calls. With 5 entries: 5 × 75pt × $100 = $37,500 ≤ $39,000.
- **Min Spread Width (MKT-026):** Call floor 60pt, put floor 75pt (MKT-028). On low-VIX days, spreads use these floors instead of formula values. MKT-008 liquidity fallback uses universal `min_spread_width=60` floor.
- **VIX-Scaled Spread Width (MKT-027):** Continuous formula `round(VIX × 3.5 / 5) × 5` with per-side floors (MKT-028). Pushes long legs further OTM on high-VIX days → cheaper longs → higher net credit → more stop cushion. Configurable via `spread_vix_multiplier` (default 3.5), capped at `max_spread_width` (75pt for margin).
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. Separate thresholds: call $1.25 (v1.17.0, raised from $0.60 — walk-forward backtest optimized), put $2.25 (v1.17.0, lowered from $2.50 — walk-forward backtest optimized). MKT-029 graduated fallback for BOTH sides: -$0.05, -$0.10 (call floor $0.50, put floor $2.40). MKT-035/MKT-038 call-only entries also use MKT-029 call floor ($0.50). Call non-viable + put viable → put-only entry, but only when VIX < 25.0 per MKT-032/MKT-039. Put non-viable + call viable → retry with tighter put strikes (5pt closer, max 2 retries), then call-only entry (MKT-040, 89% WR, stop = call + theo $2.50 put + buffer). Both non-viable → skip.
- **Conditional Entry Trigger (MKT-035 / Upday-035):** Two conditional slots fire based on intraday SPX direction from session open:
  - **E7 (13:15) — MKT-035 down-day call-only:** When SPX drops ≥ 0.3% below open, fires call-only spread. Stop = call_credit + theoretical $2.50 put + buffer. Config: `conditional_e7_enabled: true`, `downday_threshold_pct: 0.003`.
  - **E6 (12:45) — Upday-035 up-day put-only:** When SPX rises ≥ 0.4% above open, fires put-only spread. Stop = put_credit + put_stop_buffer. Config: `conditional_upday_e6_enabled: true`, `upday_threshold_pct: 0.004`.
  - Base entries E1-E5 are NOT affected by either conditional — they always attempt full ICs.
  - Down-day takes priority if both conditions met for a slot (rare edge case).
- **Progressive OTM Tightening (MKT-020 Calls / MKT-022 Puts):** For all entries, scans from MKT-024 starting distance inward in 5pt steps until credit >= minimum ($0.60 calls, $2.50 puts — with MKT-029 fallback floors $0.50/$2.40) or 25pt OTM floor reached. Each uses batch API (1 chain + 1 batch quote = 2 calls). If can't reach floor, MKT-011 skips or converts entry.
...
- **Stop Formula:** Full IC stop = total_credit + buffer (asymmetric: call uses `stop_buffer` $0.10, put uses `put_stop_buffer` $1.00 ($100 buffer)). MKT-040 call-only (put non-viable): call_credit + theoretical $2.50 put + call buffer (unified with MKT-035/038). Put-only (MKT-039): credit + $5.00 ($5.00 buffer prevents false stops, 2× redundant, max loss $750→$500). MKT-035/MKT-038 call-only (conditional/FOMC): call_credit + theoretical $2.50 put + call buffer. Configurable via `stop_buffer` and `put_stop_buffer` config keys. 21-day backtest: $5.00 put buffer avoids 91% of false put stops (+$6,885 NET improvement). MKT-019 (2× max credit) removed in v1.4.0.
- **Stop Confirmation Timer (MKT-036):** INTENTIONALLY DISABLED. The $5.00 put buffer (`put_stop_buffer`) is the chosen solution for false stops instead. Code preserved but dormant — set `stop_confirmation_enabled: true` to re-enable. When enabled: 75-second confirmation window before executing stop.
- **FOMC T+1 Call-Only (MKT-038):** On the day after FOMC announcement (T+1), forces all entries to call-only spreads. Research: T+1 is 66.7% down days with 23% more volatility — put-side exposure is dangerous. Stop formula same as MKT-035: call_credit + theoretical $2.50 put + buffer. Configurable via `fomc_t1_callonly_enabled` (default true).
- **Base-Entry Down-Day Call-Only:** When SPX is down >= `base_entry_downday_callonly_pct` from session open at E1-E5 entry time, the full IC is converted to call-only (put side skipped). Walk-forward backtest optimized at 0.4%: 32% MaxDD reduction with near-zero Sharpe cost. Distinct from MKT-035/E6/E7 — those are BONUS conditional entries that only fire when triggered; base entries always fire but become safer on down days. Stop formula: same as all call-only entries (`call_credit + theoretical $10.00 put + buffer`). Config key: `base_entry_downday_callonly_pct` (decimal fraction, e.g. `0.004` = 0.4%). Override reason in Sheets/logs: `[BASE-DOWNDAY]`. VM config: `"base_entry_downday_callonly_pct": 0.004`.
...
- **Telegram Commands (15 total):** Background daemon thread polls Telegram `getUpdates` every 5s. Credentials from Secret Manager (`calypso-telegram-credentials`). Security: only responds to configured chat_id. Commands: `/status` (bot state, market, filters), `/snapshot` (live positions, market hours only), `/entry N` (entry details), `/lastday` (last complete day), `/week` (current week summary), `/account` (lifetime performance), `/stops` (stop analysis), `/config` (current config, read-only), `/set` (edit config parameter, requires /restart to apply), `/hermes` (latest HERMES report), `/apollo` (latest APOLLO briefing), `/clio` (latest CLIO weekly analysis), `/restart` (restart HYDRA service), `/stop` (stop HYDRA, warns if active positions), `/help` (command list). Message splitting for long reports (HERMES/APOLLO/CLIO) — splits at paragraph/line boundaries with `(1/N)` headers instead of truncating at 4096 chars.
...
11. **HYDRA bot:** Running in LIVE mode (v1.18.0, deployed 2026-03-23) - ONLY active trading bot - 5 base entries only at :15/:45 marks starting 10:15 AM ET (E7 downday call-only ENABLED 0.3% threshold, E6 upday put-only ENABLED 0.4% threshold, Upday-035) (MKT-034 VIX time shifting DISABLED) + MKT-031 smart entry DISABLED (v1.10.4 — enter at scheduled times only) + MKT-035/Upday-035 conditional entries (E7 at 13:15 fires call-only when SPX drops >= 0.3% below open; E6 at 12:45 fires put-only when SPX rises >= 0.4% above open) + base-entry down-day call-only: E1-E5 convert to call-only when SPX drops >= 0.4% from open (`base_entry_downday_callonly_pct: 0.004`) + EMA 20/40 trend signal (informational only) + wider starting OTM 3.5×/4.0× multiplier (MKT-024) + 50pt spread width + credit gate: call $1.25, put $2.25 with MKT-029 graduated fallback for both sides (call floor $0.50, put floor $2.40) (MKT-011) + put-only entries when call non-viable AND VIX < 25 (MKT-032/MKT-039) + call-only entries when put non-viable (MKT-040, 89% WR) + VIX entry cutoff disabled (max_vix_entry=999) + early close DISABLED (MKT-018) + stop = total_credit + asymmetric buffer (call $0.10, put $1.00), MKT-036 confirmation timer DISABLED, MKT-039 put-only stop: credit + $1.00 put buffer, MKT-040 call-only stop: call + $2.50 theo put + call buffer, MKT-035/038 E6/E7: call + $2.50 theo put + call buffer + MKT-038 FOMC T+1 call-only (all entries forced call-only day after FOMC announcement) + configurable stop close mode (default: both legs) + progressive OTM tightening (MKT-020/MKT-022) + 15 Telegram commands + skip alerts: Telegram ENTRY_SKIPPED at all 8 skip paths with detailed reasons (v1.16.0) + state file: `entry_schedule` (base+conditional times), `skip_reason` per entry + dashboard: mobile-responsive header, pending cards with scheduled times, skipped cards with reason
