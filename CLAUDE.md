...
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. Separate thresholds: call $0.60 (v1.10.4, lowered from $0.75 — calls are secondary income, lower min keeps calls further OTM), put $2.50 (v1.10.4, raised from $1.75 — 20-day data: $2.50-$3.49 bucket = 66.7% survival +$159 EV vs $1.50-$1.99 = 48.3% +$23 EV; higher min forces MKT-022 closer to ATM into Week 1 sweet spot 42-65pt OTM). MKT-029 graduated fallback for BOTH sides: -$0.05, -$0.10 (call floor $0.50, put floor $2.40). MKT-035/MKT-038 call-only entries also use MKT-029 call floor ($0.50). Call non-viable + put viable → put-only entry, but only when VIX < 25.0 per MKT-032/MKT-039. Put non-viable + call viable → retry with tighter put strikes (5pt closer, max 2 retries), then call-only entry (MKT-040, 89% WR, stop = call + theo $2.50 put + buffer). Both non-viable → skip.
- **Conditional Entry Trigger (MKT-035):** When SPX drops >= 0.3% below the session open, conditional entries E6/E7 (12:45, 13:15) fire as call-only spreads. Base entries E1-E5 are NOT affected — they always attempt full ICs (the $5.00 put stop buffer provides sufficient down-day protection). Stop for MKT-035 call-only entries = call_credit + theoretical $2.50 put + buffer. Configurable via `downday_callonly_enabled`, `downday_threshold_pct`, `downday_theoretical_put_credit`, `conditional_entry_times`, `conditional_e6_enabled`, `conditional_e7_enabled`. **CURRENTLY DISABLED** (`conditional_e6_enabled: false`, `conditional_e7_enabled: false`) — disabled 2026-03-20 pending more data. Backtest: only 2 valid-trigger days (both Mar 13, 100% expiry), but SPX-open reference bug caused -$595 in losses on Mar 19 before fix. Re-enable once 10+ valid-trigger days accumulate under corrected SPX-open logic.
...
- **Stop Formula:** Full IC stop = total_credit + buffer (asymmetric: call uses `stop_buffer` $0.10, put uses `put_stop_buffer` $5.00). MKT-040 call-only (put non-viable): call_credit + theoretical $2.50 put + call buffer (unified with MKT-035/038). Put-only (MKT-039): credit + $5.00 ($5.00 buffer prevents false stops, 2× redundant, max loss $750→$500). MKT-035/MKT-038 call-only (conditional/FOMC): call_credit + theoretical $2.50 put + call buffer. Configurable via `stop_buffer` and `put_stop_buffer` config keys. 21-day backtest: $5.00 put buffer avoids 91% of false put stops (+$6,885 NET improvement). MKT-019 (2× max credit) removed in v1.4.0.
