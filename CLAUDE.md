...
### MEIC-TF Bot Details (v1.3.7 - Updated 2026-02-24)
...
- **Pre-Entry ROC Gate (MKT-021):** Before placing each entry (after all 5 placed), checks if ROC already exceeds early close threshold (2%). If so, skips remaining entries to prevent ROC dilution — MKT-018 fires on same cycle at the higher undiluted ROC. Only active when MKT-018 is enabled.
