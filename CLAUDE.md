...
### HYDRA Bot Details (v1.10.1 - Updated 2026-03-09)
- **Strategy:** MEIC + Trend Following Hybrid (EMA 20/40 direction filter)
- **Structure:** 5 entries per day with VIX-scaled scheduling (MKT-034). Entry execution at :14:30/:44:30 (30s before :15/:45 marks for fill precision). Default: 11:14:30-13:14:30. VIX gate shifts E#1 later on high-VIX days (VIX 20-23 → start 11:44:30, VIX >= 23 → start 12:14:30). Full iron condors or put-only (MKT-011) + credit gate.
- **VIX-Scaled Entry Time Shifting (MKT-034):** VIX gate check at :14:00/:44:00 (30s before entry) for E#1 only. VIX < 20 → default schedule. VIX 20-23 → skip first slot, start at 11:44:30. VIX >= 23 → start at 12:14:30 (floor). After E#1, E#2-E#5 use normal scheduling. MKT-031 early entry integrates: checks VIX gate first. Configurable via `vix_time_shift` config.
- **Smart Entry Windows (MKT-031):** 10-minute scouting window before each scheduled entry. Scores market conditions every main-loop cycle using 2 parameters: post-spike ATR calm (0-70pts) + momentum pause (0-30pts). Score >= 65 triggers early entry (after VIX gate check for E#1). Otherwise enters at scheduled time (default behavior preserved). Zero-risk fallback: if scoring fails, enters at scheduled time.
...
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. Separate thresholds: call $0.75 strict (v1.7.2, lowered from $1.00 — credit cushion analysis shows 68% call cushion vs 61.5%, crosses 65% safety threshold), put $1.75 (MKT-029 fallback to $1.65). Call non-viable + put viable → put-only entry (v1.7.1, 87.5% WR), but only when VIX < 18.0 per MKT-032 (at higher VIX, skips instead — 2× stop too risky without hedge, 50% WR). Put non-viable → skip (call-only disabled). Both non-viable → skip. See `docs/HYDRA_CREDIT_CUSHION_ANALYSIS.md` for full analysis.
...
- **Stop Formula:** Full IC stop = total_credit + $0.10 (Brian's credit+buffer approach — extra cushion, each stop costs ~$25 instead of breakeven). One-sided entries: stop = 2× credit + $0.10 (Fix #40 pattern). Configurable via `stop_buffer` config key. MKT-019 (2× max credit) removed in v1.4.0.
...
11. **HYDRA bot:** Running in LIVE mode (v1.10.2, deployed 2026-03-10) - ONLY active trading bot - 5 entries per day with VIX-scaled scheduling (MKT-034: VIX < 20 starts 11:14:30, VIX 20-23 starts 11:44:30, VIX >= 23 starts 12:14:30 — entry execution at :14:30/:44:30 for fill precision, VIX gate checks at :14:00/:44:00 for E#1 only, floor at 12:14:30) + MKT-031 smart entry windows (10min pre-entry scouting with VIX gate integration, 2-parameter scoring: post-spike ATR calm + momentum pause, threshold 65 triggers early entry after VIX gate check) + EMA 20/40 trend signal (informational only, all entries full IC) + wider starting OTM 3.5×/4.0× multiplier (MKT-024) + asymmetric spread widths (MKT-028: put floor 75pt, call floor 60pt) + VIX-scaled spread width (MKT-027: VIX×3.5, per-side floors, cap 75pt) + credit gate: call $0.75, put $1.75 with MKT-029 fallback (MKT-011) + put-only entries when call non-viable AND VIX < 18 (MKT-032 VIX gate; at VIX >= 18 skips instead) + early close DISABLED (MKT-018) + stop = total_credit + $0.10 (credit+buffer) + configurable stop close mode (default: both legs; `short_only_stop: true` enables MKT-025 + MKT-033 long salvage) + progressive OTM tightening (MKT-020/MKT-022) + early close day cutoff 12:30 PM (allows 12:14:30 entry on high VIX days) + 15 Telegram commands (v1.9.4: /clio weekly analysis, /set config editing, /restart, /stop with position warning, message splitting for long reports)
