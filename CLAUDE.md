...
### HYDRA Bot Details (v1.12.0 - Updated 2026-03-11)
- **Strategy:** MEIC + Trend Following Hybrid (EMA 20/40 direction filter)
- **Structure:** 5 base entries + 2 conditional entries per day at :15/:45 marks starting 10:15 AM ET (v1.10.3 — matches winning period Feb 10-27). Full iron condors, put-only (MKT-011), or call-only on down days (MKT-035) + credit gate. VIX entry cutoff disabled (max_vix_entry=999, neither Tammy nor Sandvand use VIX cutoffs). Conditional entries (12:45, 13:15) only fire when MKT-035 triggers (SPX < open -0.3%), always as call-only.
...
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. Separate thresholds: call $0.60 (v1.10.4, lowered from $0.75 — calls are secondary income, lower min keeps calls further OTM), put $2.50 (v1.10.4, raised from $1.75 — 20-day data: $2.50-$3.49 bucket = 66.7% survival +$159 EV vs $1.50-$1.99 = 48.3% +$23 EV; higher min forces MKT-022 closer to ATM into Week 1 sweet spot 42-65pt OTM). MKT-029 fallback: -$0.05, -$0.10. Call non-viable + put viable → put-only entry, but only when VIX < 18.0 per MKT-032. Put non-viable → skip. Both non-viable → skip.
- **Down Day Filter (MKT-035):** When SPX drops >= 0.3% below today's open, converts entry to call-only (no puts). Stop = call_credit + theoretical $2.50 put + buffer. 20-day data: 71% put stop rate on down days vs 7% call stop rate. Conditional entries (12:45, 13:15) only fire when MKT-035 triggers, always as call-only. Configurable via `downday_callonly_enabled`, `downday_threshold_pct`, `downday_theoretical_put_credit`, `conditional_entry_times`.
...
- **Stop Formula:** Full IC stop = total_credit + buffer (asymmetric: call uses `stop_buffer` $0.10, put uses `put_stop_buffer` $5.00). One-sided entries: stop = 2× credit + buffer (Fix #40 pattern). MKT-035 call-only: stop = call_credit + theoretical $2.50 put + call buffer. Configurable via `stop_buffer` and `put_stop_buffer` config keys. 21-day backtest: $5.00 put buffer avoids 91% of false put stops (+$6,885 NET improvement). MKT-019 (2× max credit) removed in v1.4.0.
- **Stop Confirmation Timer (MKT-036):** When spread value breaches stop level, 75-second confirmation window before executing. If spread recovers below stop level during window, timer resets (stop avoided). Prevents false stops from brief price spikes. 20-day backtest: 17 false stops avoided ($2,870 saved), 1 real stop missed ($85). Applies to both put and call sides. Timer resets on bot restart (conservative). Configurable via `stop_confirmation_enabled` (default true) and `stop_confirmation_seconds` (default 75).
...
11. **HYDRA bot:** Running in LIVE mode (v1.12.0, deployed 2026-03-11) - ONLY active trading bot - 5 base entries + 2 conditional entries at :15/:45 marks starting 10:15 AM ET (MKT-034 VIX time shifting DISABLED) + MKT-031 smart entry DISABLED (v1.10.4 — enter at scheduled times only) + MKT-035 call-only on down days (SPX < open -0.3% → call spread only, conditional entries 12:45/13:15 only fire on down days) + EMA 20/40 trend signal (informational only) + wider starting OTM 3.5×/4.0× multiplier (MKT-024) + 50pt spread width + credit gate: call $0.60, put $2.50 with MKT-029 fallback (MKT-011) + put-only entries when call non-viable AND VIX < 18 (MKT-032) + VIX entry cutoff disabled (max_vix_entry=999) + early close DISABLED (MKT-018) + stop = total_credit + asymmetric buffer (call $0.10, put $5.00) with MKT-036 75s confirmation timer, MKT-035: call + $2.50 theo put + call buffer + configurable stop close mode (default: both legs) + progressive OTM tightening (MKT-020/MKT-022) + 15 Telegram commands
