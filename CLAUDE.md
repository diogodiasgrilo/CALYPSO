...
- **Stop Formula:** Full IC stop = total_credit + $0.10 (Brian's credit+buffer approach — extra cushion, each stop costs ~$25 instead of breakeven). One-sided entries: stop = 2× credit + $0.10 (Fix #40 pattern). MKT-035 call-only: stop = call_credit + theoretical $2.50 put + $0.10 buffer. Configurable via `stop_buffer` config key. MKT-019 (2× max credit) removed in v1.4.0.
