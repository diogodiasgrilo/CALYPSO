...
### HYDRA Bot Details (v1.19.0 - Updated 2026-03-29)
- **Strategy:** MEIC + Trend Following Hybrid (EMA 20/40 direction filter)
- **Structure:** 3 base entries per day at 10:15, 10:45, 11:15 AM ET (v1.19.0 — converged from 5 entries). Full iron condors or put-only (MKT-011) + credit gate. VIX entry cutoff disabled (max_vix_entry=999, neither Tammy nor Sandvand use VIX cutoffs). Conditional entries: **E7 DISABLED** (`conditional_e7_enabled: false`). **E6 (14:00) ENABLED** as up-day put-only (`conditional_upday_e6_enabled: true`, threshold 0.48% rise from open — Upday-035). Walk-forward backtest Sharpe 3.282, realistic live Sharpe 2.684. Base entries E1-E3 always attempt full ICs regardless of market direction.
- **Anti-Whipsaw Filter:** `whipsaw_range_skip_mult = 1.50` — skips entries when intraday range exceeds 1.5× expected move (choppy/whipsaw conditions).
...
- **VIX-Scaled Spread Width (MKT-027):** Continuous formula `round(VIX × 5.3 / 5) × 5`, floor 25pt, cap 85pt. Pushes long legs further OTM on high-VIX days → cheaper longs → higher net credit → more stop cushion. Config: `call_min_spread_width=25`, `put_min_spread_width=25`, `max_spread_width=85`, `spread_vix_multiplier=5.3`.
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. Separate thresholds: call $1.35 (v1.19.0), put $2.10 (v1.19.0). MKT-029 graduated fallback for BOTH sides: -$0.05, -$0.10 (call floor $0.75, put floor $2.07). MKT-038 call-only entries also use MKT-029 call floor ($0.75). Call non-viable + put viable → put-only entry, but only when VIX < 15.0 per MKT-032/MKT-039. Put non-viable + call viable → retry with tighter put strikes (5pt closer, max 2 retries), then call-only entry (MKT-040, 89% WR, stop = call + theo $2.60 put + buffer). Both non-viable → skip.
- **Conditional Entry Trigger (Upday-035):** One conditional slot fires based on intraday SPX direction from session open:
  - **E7 — MKT-035 down-day call-only:** DISABLED (`conditional_e7_enabled: false`).
  - **E6 (14:00) — Upday-035 up-day put-only:** When SPX rises ≥ 0.48% above open, fires put-only spread. Stop = put_credit + put_stop_buffer. Config: `conditional_upday_e6_enabled: true`, `upday_threshold_pct: 0.0048`.
  - Base entries E1-E3 are NOT affected by the conditional — they always attempt full ICs.
- **Progressive OTM Tightening (MKT-020 Calls / MKT-022 Puts):** For all entries, scans from MKT-024 starting distance inward in 5pt steps until credit >= minimum ($1.35 calls, $2.10 puts — with MKT-029 fallback floors $0.75/$2.07) or 25pt OTM floor reached. Each uses batch API (1 chain + 1 batch quote = 2 calls). If can't reach floor, MKT-011 skips or converts entry.
...
- **Stop Formula:** Full IC stop = total_credit + buffer (asymmetric: call uses `call_stop_buffer` $0.35, put uses `put_stop_buffer` $1.55). MKT-040 call-only (put non-viable): call_credit + theoretical $2.60 put + call buffer (unified with MKT-038). Put-only (MKT-039): credit + put_stop_buffer ($1.55). MKT-038 call-only (FOMC T+1): call_credit + theoretical $2.60 put + call buffer. Configurable via `call_stop_buffer` and `put_stop_buffer` config keys. MKT-019 (2× max credit) removed in v1.4.0.
- **Stop Confirmation Timer (MKT-036):** INTENTIONALLY DISABLED. The put buffer (`put_stop_buffer` $1.55) is the chosen solution for false stops instead. Code preserved but dormant — set `stop_confirmation_enabled: true` to re-enable. When enabled: 75-second confirmation window before executing stop.
...
- **FOMC T+1 Call-Only (MKT-038):** On the day after FOMC announcement (T+1), forces all entries to call-only spreads. Research: T+1 is 66.7% down days with 23% more volatility — put-side exposure is dangerous. Stop formula: call_credit + theoretical $2.60 put + call buffer. Configurable via `fomc_t1_callonly_enabled` (default true).
- **Base-Entry Down-Day Call-Only:** When SPX is down >= `base_entry_downday_callonly_pct` from session open at E1-E3 entry time, the full IC is converted to call-only (put side skipped). Walk-forward backtest optimized at 0.57%. Distinct from E6 — E6 is a BONUS conditional entry that only fires when triggered; base entries always fire but become safer on down days. Stop formula: same as all call-only entries (`call_credit + theoretical $2.60 put + call buffer`). Config key: `base_entry_downday_callonly_pct` (decimal fraction, e.g. `0.0057` = 0.57%). Override reason in Sheets/logs: `[BASE-DOWNDAY]`. VM config: `"base_entry_downday_callonly_pct": 0.0057`.
...
11. **HYDRA bot:** Running in LIVE mode (v1.19.0, deployed 2026-03-29) - ONLY active trading bot - 3 base entries at 10:15, 10:45, 11:15 AM ET (E7 downday call-only DISABLED, E6 upday put-only ENABLED at 14:00 with 0.48% threshold, Upday-035) (MKT-034 VIX time shifting DISABLED) + MKT-031 smart entry DISABLED (v1.10.4 — enter at scheduled times only) + Upday-035 conditional entry (E6 at 14:00 fires put-only when SPX rises >= 0.48% above open) + anti-whipsaw filter (`whipsaw_range_skip_mult: 1.50`) + base-entry down-day call-only: E1-E3 convert to call-only when SPX drops >= 0.57% from open (`base_entry_downday_callonly_pct: 0.0057`) + EMA 20/40 trend signal (informational only) + wider starting OTM 3.5x/4.0x multiplier (MKT-024) + VIX-scaled spread width 25-85pt (MKT-027, floor=25 cap=85 mult=5.3) + credit gate: call $1.35, put $2.10 with MKT-029 graduated fallback for both sides (call floor $0.75, put floor $2.07) (MKT-011) + put-only entries when call non-viable AND VIX < 15 (MKT-032/MKT-039) + call-only entries when put non-viable (MKT-040, 89% WR) + VIX entry cutoff disabled (max_vix_entry=999) + early close DISABLED (MKT-018) + stop = total_credit + asymmetric buffer (call $0.35, put $1.55), MKT-036 confirmation timer DISABLED, MKT-039 put-only stop: credit + $1.55 put buffer, MKT-040 call-only stop: call + $2.60 theo put + call buffer, MKT-038: call + $2.60 theo put + call buffer + MKT-038 FOMC T+1 call-only (all entries forced call-only day after FOMC announcement) + FOMC announcement skip DISABLED + configurable stop close mode (default: both legs) + progressive OTM tightening (MKT-020/MKT-022) + 15 Telegram commands + skip alerts: Telegram ENTRY_SKIPPED at all 8 skip paths with detailed reasons (v1.16.0) + state file: `entry_schedule` (base+conditional times), `skip_reason` per entry + dashboard: mobile-responsive header, pending cards with scheduled times, skipped cards with reason + backtesting: 1-minute data resolution, Sharpe 3.282 (realistic live 2.684)
