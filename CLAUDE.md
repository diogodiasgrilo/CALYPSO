...
### HYDRA Bot Details (v1.10.3 - Updated 2026-03-11)
- **Strategy:** MEIC + Trend Following Hybrid (EMA 20/40 direction filter)
- **Structure:** 5 entries per day at :15/:45 marks starting 10:15 AM ET (v1.10.3 — matches winning period Feb 10-27). Full iron condors or put-only (MKT-011) + credit gate. VIX entry cutoff disabled (max_vix_entry=999, neither Tammy nor Sandvand use VIX cutoffs).
...
11. **HYDRA bot:** Running in LIVE mode (v1.10.3, deployed 2026-03-11) - ONLY active trading bot - 5 entries per day at :15/:45 marks starting 10:15 AM ET (MKT-034 VIX time shifting DISABLED — neither Tammy nor Sandvand use VIX-based scheduling) + MKT-031 smart entry windows (10min pre-entry scouting, 2-parameter scoring: post-spike ATR calm + momentum pause, threshold 65 triggers early entry) + EMA 20/40 trend signal (informational only, all entries full IC) + wider starting OTM 3.5×/4.0× multiplier (MKT-024) + 50pt spread width (reverted from VIX-scaled MKT-027/028 — winning period used 50pt) + credit gate: call $0.75, put $1.75 with MKT-029 fallback (MKT-011) + put-only entries when call non-viable AND VIX < 18 (MKT-032 VIX gate; at VIX >= 18 skips instead) + VIX entry cutoff disabled (max_vix_entry=999) + early close DISABLED (MKT-018) + stop = total_credit + $0.10 (credit+buffer) + configurable stop close mode (default: both legs; `short_only_stop: true` enables MKT-025 + MKT-033 long salvage) + progressive OTM tightening (MKT-020/MKT-022) + early close day cutoff 12:30 PM + 15 Telegram commands (v1.9.4: /clio weekly analysis, /set config editing, /restart, /stop with position warning, message splitting for long reports)
