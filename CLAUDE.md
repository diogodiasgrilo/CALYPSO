...
### HYDRA Bot Details (v1.22.0 - Updated 2026-04-02)
...
- **VIX Regime Adaptive (updated 2026-04-13):** Adjusts entries and credit thresholds based on VIX at open. Current breakpoints `[18.0, 22.0, 28.0]` (previously `[14, 20, 30]`): VIX<18 = 3 entries (calm market), VIX 18-22 = 2 entries (drop E#1), VIX 22-28 = 2 entries + lower credits ($0.75 call / $1.25 put) for further OTM, VIX≥28 = 1 entry + lowest credits ($0.50/$0.75) for max OTM. Config: `vix_regime` section in strategy config. Code: `strategy.py:7588` drops EARLIEST entries when capped (keeps best-performing E#3 at 11:15).
...
- **Buffer Decay (MKT-042):** Time-decaying stop buffer. Starts at `buffer_decay_start_mult` (default 2.10) times the normal buffer, linearly decays to 1x over `buffer_decay_hours` (default 2.0 hours). Wider stops early when premium is rich and moves are noisy, normal stops later as theta decays. Applied to both call and put buffers. Set `buffer_decay_start_mult` to 1.0 or null to disable. Config: `buffer_decay_start_mult`, `buffer_decay_hours` in strategy section.
