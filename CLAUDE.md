...
### HYDRA Bot Details (v1.7.1 - Updated 2026-03-03)
- **Strategy:** MEIC + Trend Following Hybrid (EMA 20/40 direction filter)
- **Structure:** 5 entries per day (10:05, 10:35, 11:05, 11:35, 12:05) — full iron condors or put-only (MKT-011) + credit gate. Entry #6 dropped to free margin for wider put spreads.
...
- **Asymmetric Spread Widths (MKT-028):** Put spread floor raised to 75pt (call floor stays 60pt). Put longs cost 7× more than calls due to skew — wider put spreads push longs further OTM = cheaper. Since MKT-025 never closes longs, this is pure savings. `margin = max(call, put) × $100`, so wider puts don't require wider calls. With 5 entries: 5 × 75pt × $100 = $37,500 ≤ $39,000.
...
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. Separate thresholds: call $0.75 strict (v1.7.2, lowered from $1.00 — credit cushion analysis shows 68% call cushion vs 61.5%, crosses 65% safety threshold), put $1.75 (MKT-029 fallback to $1.65). Call non-viable + put viable → put-only entry (v1.7.1, 87.5% WR). Put non-viable → skip (call-only disabled). Both non-viable → skip. See `docs/HYDRA_CREDIT_CUSHION_ANALYSIS.md` for full analysis.
- **Progressive OTM Tightening (MKT-020 Calls / MKT-022 Puts):** For all entries, scans from MKT-024 starting distance inward in 5pt steps until credit >= minimum ($0.75 calls, $1.75 puts) or 25pt OTM floor reached. Each uses batch API (1 chain + 1 batch quote = 2 calls). If can't reach minimum, MKT-011 skips entry.
- **Early Close (MKT-018):** After all entries placed, monitors Return on Capital (ROC). When ROC >= 3%, checks MKT-023 hold check before closing. Locks in profit on high-ROC days to prevent late-day reversals. ROC = (net_pnl - close_cost) / capital_deployed. Close cost: $5/position ($2.50 commission + $2.50 slippage). Threshold raised from 2% to 3% on Feb 25 based on 11-day analysis showing 2% left $1,025 on table with zero reversals after trigger.
- **Smart Hold Check (MKT-023):** When MKT-018 ROC threshold is met, compares close-now P&L vs worst-case-hold P&L. Determines market lean from average cushion, assumes stressed side gets stopped and safe side expires worthless. If worst-case hold > close now → HOLD. Falls through to close when no clear lean or close-now is better.
- **Pre-Entry ROC Gate (MKT-021):** Before placing entries #4 and #5 (after 3 entries placed), checks if ROC already exceeds early close threshold (3%). If so, skips remaining entries to prevent ROC dilution — MKT-018 fires on same cycle at the higher undiluted ROC. Only active when MKT-018 is enabled.
...
- **Telegram Commands (11 total):** Background daemon thread polls Telegram `getUpdates` every 5s. Credentials from Secret Manager (`calypso-telegram-credentials`). Security: only responds to configured chat_id. Commands: `/status` (bot state, market, filters), `/snapshot` (live positions, market hours only), `/entry N` (entry details), `/lastday` (last complete day), `/week` (current week summary), `/account` (lifetime performance), `/stops` (stop analysis), `/config` (current config), `/hermes` (latest HERMES report), `/apollo` (latest APOLLO briefing), `/help` (command list).
...
11. **HYDRA bot:** Running in LIVE mode (v1.7.2, deployed 2026-03-03) - ONLY active trading bot - 5 entries per day (10:05-12:05, Entry #6 dropped to free margin for wider put spreads) + EMA 20/40 trend signal (informational only, all entries full IC) + wider starting OTM 3.5×/4.0× multiplier (MKT-024: batch API = zero extra cost, catches extra cushion on high-credit days) + asymmetric spread widths (MKT-028: put floor 75pt, call floor 60pt — put longs cost 7× more due to skew, wider = cheaper = pure savings since MKT-025 never closes longs) + VIX-scaled spread width continuous formula (MKT-027: VIX×3.5 rounded to 5pt, per-side floors via MKT-028, cap 75pt for margin) + credit gate with separate thresholds: call $0.75 strict (v1.7.2, lowered from $1.00 — 68% call cushion vs 61.5%, see HYDRA_CREDIT_CUSHION_ANALYSIS.md), put $1.75 with MKT-029 fallback to $1.65 (MKT-011) + put-only entries when call non-viable (v1.7.1: 87.5% WR, call-only disabled) + early close on ROC >= 3% (MKT-018) + smart hold check before early close (MKT-023) + pre-entry ROC gate after 3 entries (MKT-021) + stop formula = total_credit - $0.15 (MEIC+ covers commission for true breakeven) + short-only stop close (MKT-025: close short, let long expire at settlement) + progressive OTM tightening for all entries (MKT-020/MKT-022) + HERMES triggered post-settlement (replaces fixed 5 PM timer) + proper P&L tracking (Fix #46/#47) + accurate log labels (Fix #49) + same-strike overlap prevention (Fix #50/MKT-013) + liquidity re-check after overlap adjustment (MKT-014) + Google Sheets timeout protection (Fix #64) + recovery entry classification fix (Fix #65) + post-MKT-013 strike conflict re-check (Fix #66) + long-long overlap prevention (Fix #67/MKT-015) + comprehensive timeout protection for all blocking calls (Fix #68) + accurate fill price tracking (Fix #70) + duplicate daily summary prevention (Fix #71) + net P&L in daily summary (Fix #72) + active entries property fix (Fix #73) + stop loss fill price deferred lookup fix (Fix #74) + async deferred stop fill lookup (Fix #75) + correct Saxo API field name for fill prices + closedpositions fallback (Fix #76) + post-restart settlement expired credits fix (Fix #77) + stop_loss_debits derived from P&L identity (Fix #78) + entries_skipped counter for MKT-011 skip (Fix #79) + batch quote API 7x rate limit reduction + Fix #80 Sheets Positions resize + min credit raised to $0.75/side calls, $1.75/side puts (v1.7.2) + skip $0 long legs during early close (Fix #81) + removed MKT-016/017/base loss limit (v1.3.3) + Fix #82 midnight settlement gate lock (v1.3.4) + cumulative ROC columns in Daily Summary (v1.3.10)
