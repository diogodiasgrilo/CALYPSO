...
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. Separate thresholds: call $0.60 (v1.10.4, lowered from $0.75 — calls are secondary income, lower min keeps calls further OTM), put $2.50 (v1.10.4, raised from $1.75 — 20-day data: $2.50-$3.49 bucket = 66.7% survival +$159 EV vs $1.50-$1.99 = 48.3% +$23 EV; higher min forces MKT-022 closer to ATM into Week 1 sweet spot 42-65pt OTM). MKT-029 fallback: -$0.05, -$0.10. Call non-viable + put viable → put-only entry, but only when VIX < 25.0 per MKT-032/MKT-039. Put non-viable + call viable → retry with tighter put strikes (5pt closer, max 2 retries), then call-only entry (MKT-040, 89% WR, stop = call + theo $2.50 put + buffer). Both non-viable → skip.
...
- **Progressive OTM Tightening (MKT-020 Calls / MKT-022 Puts):** For all entries, scans from MKT-024 starting distance inward in 5pt steps until credit >= minimum ($0.60 calls, $2.50 puts) or 25pt OTM floor reached. Each uses batch API (1 chain + 1 batch quote = 2 calls). If can't reach minimum, MKT-011 skips entry.
