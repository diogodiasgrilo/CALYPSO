...
### HYDRA Bot Details (v1.23.0 - Updated 2026-04-16)
...
- **VIX Regime Adaptive (updated 2026-04-14):** Adjusts entries and credit thresholds based on VIX at open. Breakpoints `[18.0, 22.0, 28.0]` define 4 zones. All regime slots now filled — base `min_viable_credit_per_side` ($2.00) and `min_viable_credit_put_side` ($2.75) are effectively dead because the regime always overrides. Live VM values:

  | Zone | VIX | Max entries | Min call credit | Min put credit |
  |------|------|-------------|-----------------|----------------|
  | 0 | < 18 | 3 (default) | $1.00 | $1.25 |
  | 1 | 18 – 22 | 2 (drops E#1) | $0.50 | $0.75 |
  | 2 | 22 – 28 | 2 (drops E#1) | $0.30 | $0.50 |
  | 3 | ≥ 28 | 1 (E#3 only) | $0.30 | $0.40 |
...
- **Stop Formula:** Full IC stop = total_credit + buffer (asymmetric: call uses `call_stop_buffer` $0.75, put uses `put_stop_buffer` $1.75). MKT-040 call-only (put non-viable): call_credit + theoretical $2.60 put + call buffer (unified with MKT-038). Put-only (MKT-039): credit + put_stop_buffer ($1.75). MKT-038 call-only (FOMC T+1): call_credit + theoretical $2.60 put + call buffer. Configurable via `call_stop_buffer` and `put_stop_buffer` config keys. MKT-019 (2× max credit) removed in v1.4.0.
