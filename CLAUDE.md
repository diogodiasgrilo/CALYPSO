...
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. Separate thresholds: call $0.60 (v1.10.4, lowered from $0.75 — calls are secondary income, lower min keeps calls further OTM), put $2.50 (v1.10.4, raised from $1.75 — 20-day data: $2.50-$3.49 bucket = 66.7% survival +$159 EV vs $1.50-$1.99 = 48.3% +$23 EV; higher min forces MKT-022 closer to ATM into Week 1 sweet spot 42-65pt OTM). MKT-029 fallback: -$0.05, -$0.10. Call non-viable + put viable → put-only entry, but only when VIX < 25.0 per MKT-032/MKT-039. Put non-viable + call viable → call-only entry (MKT-040, 89% WR, stop = 2× credit + $0.10). Both non-viable → skip.
...
- **Stop Formula:** Full IC stop = total_credit + buffer (asymmetric: call uses `stop_buffer` $0.10, put uses `put_stop_buffer` $5.00). MKT-040 call-only (put non-viable): 2× credit + $0.10 (Fix #40 — $0.10 too small without 2×). Put-only (MKT-039): credit + $5.00 ($5.00 buffer prevents false stops, 2× redundant, max loss $750→$500). MKT-035/MKT-038 call-only (conditional/FOMC): call_credit + theoretical $2.50 put + call buffer. Configurable via `stop_buffer` and `put_stop_buffer` config keys. 21-day backtest: $5.00 put buffer avoids 91% of false put stops (+$6,885 NET improvement). MKT-019 (2× max credit) removed in v1.4.0.
