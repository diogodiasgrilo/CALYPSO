...
- **Min Spread Width (MKT-026):** Floor raised from 25pt to 60pt. On low-VIX days (< 20), spreads use 60pt instead of 40-50pt. Long legs end up 10pt further OTM = cheaper. Since MKT-025 never closes longs (they expire at settlement), this is pure savings with no downside.
- **VIX-Scaled Spread Width (MKT-027):** Continuous formula `max(60, round(VIX × 3.5 / 5) × 5)` replaces MEIC step function. Pushes long legs further OTM on high-VIX days → cheaper longs → higher net credit → more stop cushion. VIX 22 = 75pt, VIX 25 = 90pt. Configurable via `spread_vix_multiplier` (default 3.5), capped at `max_spread_width` (default 120pt).
...
- **Pre-Entry ROC Gate (MKT-021):** Before placing entries #4, #5, and #6 (after 3 entries placed), checks if ROC already exceeds early close threshold (3%). If so, skips remaining entries to prevent ROC dilution — MKT-018 fires on same cycle at the higher undiluted ROC. Only active when MKT-018 is enabled.
