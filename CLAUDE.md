...
### HYDRA Bot Details (v1.15.0 - Updated 2026-03-16)
...
- **Stop Formula:** Full IC stop = total_credit + buffer (asymmetric: call uses `stop_buffer` $0.10, put uses `put_stop_buffer` $5.00). Call-only legacy: 2× credit + $0.10 (Fix #40 — $0.10 too small without 2×). Put-only (MKT-039): credit + $5.00 ($5.00 buffer prevents false stops, 2× redundant, max loss $750→$500). MKT-035/MKT-038 call-only: call_credit + theoretical $2.50 put + call buffer. Configurable via `stop_buffer` and `put_stop_buffer` config keys. 21-day backtest: $5.00 put buffer avoids 91% of false put stops (+$6,885 NET improvement). MKT-019 (2× max credit) removed in v1.4.0.
