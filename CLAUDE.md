...
### MEIC-TF Bot Details (v1.4.0 - Updated 2026-02-27)
...
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. Skips entry if either side is below $1.00/side minimum (no one-sided entries). Min credit: $1.00/side (MEIC-TF override; base MEIC uses $0.50).
- **Progressive OTM Tightening (MKT-020 Calls / MKT-022 Puts):** For all entries, moves short call (MKT-020) and short put (MKT-022) closer to ATM in 5pt steps until credit >= $1.00/side or 25pt OTM floor reached. Each uses batch API (1 chain + 1 batch quote = 2 calls). If can't reach $1.00, MKT-011 skips entry.
