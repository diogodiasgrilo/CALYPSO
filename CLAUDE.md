...
### MEIC-TF Bot Details (v1.3.9 - Updated 2026-02-25)
...
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. One-sided entries only allowed for clear trends (BULLISH/BEARISH >= 0.2% EMA). In NEUTRAL markets, skips entry if either side non-viable. In trending markets, converts if trend matches viable side, skips otherwise. Min credit: $1.00/side (MEIC-TF override; base MEIC uses $0.50).
- **Progressive OTM Tightening (MKT-020 Calls / MKT-022 Puts):** For full IC entries in NEUTRAL markets, moves short call (MKT-020) and short put (MKT-022) closer to ATM in 5pt steps until credit >= $1.00/side or 25pt OTM floor reached. Each uses batch API (1 chain + 1 batch quote = 2 calls). If can't reach $1.00, MKT-011 skips entry (tightening only runs for NEUTRAL, which always skips).
...
- **Smart Hold Check (MKT-023):** When MKT-018 ROC threshold is met, compares close-now P&L vs worst-case-hold P&L. Determines market lean from average cushion, assumes stressed side gets stopped and safe side expires worthless. If worst-case hold > close now → HOLD. Falls through to close when no clear lean, all one-sided, or close-now is better.
- **Pre-Entry ROC Gate (MKT-021):** Before placing entries #4 and #5 (after 3 entries placed), checks if ROC already exceeds early close threshold (3%). If so, skips remaining entries to prevent ROC dilution — MKT-018 fires on same cycle at the higher undiluted ROC. Only active when MKT-018 is enabled. Gate=3 ensures MKT-021 actually runs (gate=5 in a 5-entry system was effectively disabled).
- **Virtual Equal Credit Stop (MKT-019):** For full IC entries, stop_level = 2 × max(call_credit, put_credit) instead of total_credit. Volatility skew makes puts 2-7× more expensive than calls; using total_credit caused premature stops on the low-credit side. One-sided entries unchanged (already use 2× credit).
...
11. **MEIC-TF bot:** Running in LIVE mode (v1.3.11, deployed 2026-02-25) - ONLY active trading bot - adds EMA 20/40 trend filter + credit gate (MKT-011, one-sided only for clear trends) + early close on ROC >= 3% (MKT-018, raised from 2% based on 11-day analysis) + smart hold check before early close (MKT-023) + pre-entry ROC gate after 3 entries (MKT-021) + proper P&L tracking (Fix #46/#47) + accurate log labels (Fix #49) + same-strike overlap prevention (Fix #50/MKT-013) + liquidity re-check after overlap adjustment (MKT-014) + Google Sheets timeout protection (Fix #64) + recovery entry classification fix (Fix #65) + post-MKT-013 strike conflict re-check (Fix #66) + long-long overlap prevention (Fix #67/MKT-015) + comprehensive timeout protection for all blocking calls (Fix #68) + accurate fill price tracking (Fix #70) + duplicate daily summary prevention (Fix #71) + net P&L in daily summary (Fix #72) + active entries property fix (Fix #73) + stop loss fill price deferred lookup fix (Fix #74) + async deferred stop fill lookup (Fix #75) + correct Saxo API field name for fill prices + closedpositions fallback (Fix #76) + post-restart settlement expired credits fix (Fix #77) + stop_loss_debits derived from P&L identity (Fix #78) + entries_skipped counter for MKT-011 skip (Fix #79) + Daily Summary column redesign with OHLC and P&L breakdown (v1.2.7) + EMA neutral threshold widened to 0.2% (v1.2.8) + virtual equal credit stop for full ICs (MKT-019) + batch quote API 7x rate limit reduction + Fix #80 Sheets Positions resize + progressive call OTM tightening (MKT-020) + progressive put OTM tightening (MKT-022) + min credit raised to $1.00/side (v1.3.1) + skip $0 long legs during early close (Fix #81) + removed MKT-016/017/base loss limit (v1.3.3) + Fix #82 midnight settlement gate lock (v1.3.4) + cumulative ROC columns in Daily Summary (v1.3.10) + MKT-018 threshold raised to 3% (v1.3.11)
...
66. **~~Daily Loss Limit (MKT-017)~~ REMOVED in v1.3.3 (2026-02-23)** - MKT-016 (stop cascade breaker) and MKT-017 (daily loss limit) were both removed. Bot now always attempts all 5 entries regardless of stop count or realized P&L. Base MEIC's percentage-based loss limit also overridden to return False in MEIC-TF.
