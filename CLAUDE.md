...
### HYDRA Bot Details (v1.6.0 - Updated 2026-03-02)
- **Strategy:** MEIC + Trend Following Hybrid (EMA 20/40 direction filter)
- **Structure:** 5 entries per day (10:05, 10:35, 11:05, 11:35, 12:05) — always full iron condors + credit gate. Entry #6 dropped to free margin for wider put spreads.
...
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. Separate thresholds: call $1.00, put $1.75 (Tammy's range). Skips entry if either side below its minimum (no one-sided entries).
- **Progressive OTM Tightening (MKT-020 Calls / MKT-022 Puts):** For all entries, scans from MKT-024 starting distance inward in 5pt steps until credit >= minimum ($1.00 calls, $1.75 puts) or 25pt OTM floor reached. Each uses batch API (1 chain + 1 batch quote = 2 calls). If can't reach minimum, MKT-011 skips entry.
