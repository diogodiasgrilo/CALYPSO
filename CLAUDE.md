...
### HYDRA Bot Details (v1.10.4 - Updated 2026-03-11)
...
11. **HYDRA bot:** Running in LIVE mode (v1.10.4, deployed 2026-03-11) - ONLY active trading bot - 5 entries per day at :15/:45 marks starting 10:15 AM ET (MKT-034 VIX time shifting DISABLED) + MKT-031 smart entry DISABLED (v1.10.4 — enter at scheduled times only) + EMA 20/40 trend signal (informational only, all entries full IC) + wider starting OTM 3.5×/4.0× multiplier (MKT-024) + 50pt spread width + credit gate: call $0.60, put $2.50 with MKT-029 fallback (MKT-011, v1.10.4 — 20-day data: $2.50+ bucket = 66.7% survival, +$159 EV; higher put min forces MKT-022 closer to ATM into Week 1 sweet spot 42-65pt OTM) + put-only entries when call non-viable AND VIX < 18 (MKT-032) + VIX entry cutoff disabled (max_vix_entry=999) + early close DISABLED (MKT-018) + stop = total_credit + $0.10 (credit+buffer) + configurable stop close mode (default: both legs) + progressive OTM tightening (MKT-020/MKT-022) + 15 Telegram commands
