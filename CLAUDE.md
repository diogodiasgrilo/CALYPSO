...
- **Credit Gate (MKT-011):** Estimates credit from quotes BEFORE placing orders. In NEUTRAL markets, converts to one-sided if one side non-viable. In trending markets, skips entry if preferred side non-viable (respects trend filter). Min credit: $1.00/side (MEIC-TF override; base MEIC uses $0.50).
- **Progressive Call OTM Tightening (MKT-020):** For full IC entries in NEUTRAL markets, moves short call closer to ATM in 5pt steps until credit >= $1.00/side or 25pt OTM floor reached. Uses batch API (1 chain + 1 batch quote = 2 calls). If can't reach $1.00, MKT-011 converts to put-only.
- **Stop Cascade Breaker (MKT-016):** After N total stops (call+put) in a day, pause all remaining entries. Prevents compounding losses on whipsaw/reversal days. Default threshold: 3 stops.
- **Daily Loss Limit (MKT-017):** After realized P&L drops below -$500, pause all remaining entries. Complements MKT-016 by catching few large-slippage stops instead of many small ones. Default threshold: $500.
- **Early Close (MKT-018):** After all entries placed, monitors Return on Capital (ROC). When ROC >= 2%, closes ALL positions via market orders and logs daily summary immediately. Locks in profit on high-ROC days to prevent late-day reversals. ROC = (net_pnl - close_cost) / capital_deployed. Close cost: $5/position ($2.50 commission + $2.50 slippage).
- **Pre-Entry ROC Gate (MKT-021):** Before placing each entry (after min 2 placed), checks if ROC already exceeds early close threshold (2%). If so, skips remaining entries to prevent ROC dilution — MKT-018 fires on same cycle at the higher undiluted ROC. Only active when MKT-018 is enabled.
...
66. **Daily Loss Limit Pauses Entries After -$500 Realized P&L (MKT-017, 2026-02-18)** - Complements MKT-016 (stop cascade breaker). MKT-016 counts the NUMBER of stops (catches whipsaw/cascade days). MKT-017 checks the MAGNITUDE of realized P&L (catches days with few but high-slippage stops). Default threshold: -$500, configurable via `strategy.max_daily_loss`. Threshold calibrated against 6 days of baseline data: Feb 13's intraday trough was -$450, so -$500 provides $50 buffer while not triggering on any profitable day. State persisted to disk and restored on restart (same pattern as MKT-016). During market hours, `total_realized_pnl` only reflects stop losses (expired credits added at settlement), so the threshold effectively measures intraday stop loss magnitude. (Cost: Prevention — would have prevented additional losses on catastrophic days beyond -$500)
