...
- **Buffer Decay (MKT-042):** Time-decaying stop buffer. Starts at `buffer_decay_start_mult` (default 1.75) times the normal buffer, linearly decays to 1x over `buffer_decay_hours` (default 2.0 hours). Wider stops early when premium is rich and moves are noisy, normal stops later as theta decays. Applied to both call and put buffers. Set `buffer_decay_start_mult` to 1.0 or null to disable. Config: `buffer_decay_start_mult`, `buffer_decay_hours` in strategy section.
