...
- **VIX Regime Adaptive:** Adjusts entries and buffers based on VIX at open. Breakpoints [14, 20, 30]: VIX<14 = 2 entries + $1.25 put buffer, VIX 14-30 = 3 entries normal buffers, VIX≥30 = 1 entry. Config: `vix_regime` section in strategy config.
...
- **Conditional Entry Trigger (Upday-035):** One conditional slot fires based on intraday SPX direction from session open:
  ...
  - **E6 (14:00) — Upday-035 up-day put-only:** When SPX rises ≥ 0.25% above open, fires put-only spread. Stop = put_credit + put_stop_buffer. Config: `conditional_upday_e6_enabled: true`, `upday_threshold_pct: 0.0025`.
  - Base entries E1-E3 are NOT affected by the conditional — they always attempt full ICs.
- **Progressive OTM Tightening (MKT-020 Calls / MKT-022 Puts):** For all entries, scans from MKT-024 starting distance inward in 5pt steps until credit >= minimum ($2.00 calls, $2.75 puts — with MKT-029 fallback floors $0.75/$2.00) or 25pt OTM floor reached. Each uses batch API (1 chain + 1 batch quote = 2 calls). If can't reach floor, MKT-011 skips or converts entry.
...
- **Cushion Recovery Exit (MKT-041):** Closes individual IC sides when they nearly hit their stop (>= `cushion_nearstop_pct` of stop level, default 96%) then recover (<= `cushion_recovery_pct` of stop level, default 67%). Disabled by default (both config keys null). Backtest: Sharpe 2.182 vs 2.094 baseline over 938 days, fires ~10.8% of days. Config: `cushion_nearstop_pct`, `cushion_recovery_pct` in strategy section.
