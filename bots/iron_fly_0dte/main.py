...
# Note: MarketStatusMonitor is enabled in delta_neutral to avoid duplicate alerts
...
# to trade. Does NOT affect the shared `strategy.py` module — MEIC's
# strategy module is HYDRA's parent class and must remain importable.
#
...
def _check_disabled_kill_switch():
    ...
    banner = (
        "\n" + "=" * 72 +
        "\n  \u26d4 IRON FLY BOT DISABLED FOR SAFETY \u2014 refusing to start" +
        "\n" + "=" * 72 +
        "\n  Per user directive (2026-04-21), HYDRA is the ONLY bot that runs." +
        "\n" +
        "\n  To re-enable:" +
        "\n    Edit bots/iron_fly_0dte/main.py" +
...
def run_bot(config: dict, dry_run: bool = False, check_interval: int = 5):
    ...
    # Note: MarketStatusMonitor (countdown/open/close alerts) is enabled in
    # delta_neutral bot to avoid duplicate alerts when multiple bots are running
    ...
    # REST-ONLY MODE: WebSocket streaming disabled for reliability
    # Reason: The strategy already uses skip_cache=True for all price fetches (see strategy.py
    # update_market_data), making WebSocket subscriptions redundant overhead. The CfdOnIndex
    # instruments (US500.I) don't receive reliable delta updates via WebSocket anyway.
    # All price updates come from REST API calls during run_strategy_check().
    trade_logger.log_event("Using REST-only mode for price fetching (WebSocket disabled)")
    ...
    status_interval = 15  # Log status every 15 seconds for 0DTE
    ...
    try:
        while not shutdown_requested:
            try:
                ...
                # Run strategy check (uses REST API for all price fetching)
                ...
                # Periodic status logging (every 15 seconds for 0DTE fast updates)
                now = datetime.now()
...
Examples:
  ...
  python -m bots.iron_fly_0dte.main --calibrate 25   Use 25-point expected move
    ...
    try:
        ...
        # CLI flag takes priority, then config file, default is False
