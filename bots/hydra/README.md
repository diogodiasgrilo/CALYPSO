...
- **1.27.2** (2026-05-05 PM): Hedge position tracking for the defensive overlay — closes the last dry-run shortcut. New `bots/hydra/brandon/hedge_position.py` provides `HedgeLeg` (dataclass with `position_id`, `fill_price`, structure metadata), `black_scholes_call_price` / `black_scholes_put_price`, `estimate_fill_price` (BS pricer using cached spot + default 18% IV for SPX 0DTE), `leg_pnl_at_expiry` (long: +(intrinsic − fill); short: +(fill − intrinsic), × 100 × quantity), and `settle_hedge` (aggregates a list of HedgeLegs into a `HedgeSettlement` with `total_pnl` and `total_debit_paid`). `BrandonHydraStrategy._brandon_place_overlay` now creates HedgeLeg objects with synthetic `DRY_OVERLAY_<entry>_<side>_<i>` position ids in dry-run (real Saxo ids in live mode via `_place_option_order`), stored on `self._brandon_hedge_legs[entry_number]`. New `_brandon_settle_hedges(spx_settle)` computes per-hedge net P&L vs SPX_close, fires per-hedge `BRANDON-OVERLAY-SETTLED` Telegram (POSITION_CLOSED, MEDIUM) and a day-aggregate `BRANDON-OVERLAY-DAY` total. `log_daily_summary` override calls settlement before parent super(). Idempotent within day; reset in `_reset_for_new_day`. **Tests**: 166 passing (was 138). Added 23 hedge-math tests + 5 hedge-integration tests. Now your 4-week dry-run experiment actually measures overlay P&L impact, not just whether the rule fired.

- **1.27.1** (2026-05-05): Brandon stack promoted to **fully live** in variants B/C — removes the "shadow" framing on GEX strike adjuster, breach exit, and defensive overlay. All three now act in dry-run mode and would act in live mode when `dry_run` is flipped off. **The only shadow behavior** is HYDRA's existing credit+buffer stop, which runs in parallel as a counterfactual: every tick computes whether HYDRA's stop *would* fire (`spread_value >= side_stop`), and the first fire per side per day is Telegrammed for head-to-head comparison vs. Brandon's GEX breach (which is now the live stop in B/C). HYDRA's stop never closes a position in B/C. **Concrete changes:** (a) `_brandon_apply_strike_adjuster` mutates `entry.short_*_strike` and recomputes `entry.long_*_strike` at the existing spread width before `_execute_entry` / `_simulate_entry`; SKIP routes through HYDRA's existing one-sided entry path (sets `*_side_skipped` and `*_only`). (b) `_brandon_check_breach_exit` calls `_close_entry_early` on confirmed 90s breach, marking `*_side_pivot_closed`. Wall filter is anchored to the entry's short strike (not current spot) so the wall stays relevant even after spot has moved past it — fixed a bug where the breach signal would die at the worst time. (c) `_brandon_place_overlay` places hedge legs via `_place_option_order` in live mode; in dry-run mode logs legs to journal + Telegram (no Saxo round-trip per `SAFETY-DRY-01`). (d) GEX cache: 1-day TTL → 15-min refresh + 60s failure cooldown. (e) Promoted to config: `gex.{decel_min_pct: 0.05, accel_min_pct: 0.10, max_shift_pts: 25, shift_buffer_pts: 5}`, `defensive_overlay.{butterfly_cutoff_hour: 12, butterfly_cutoff_minute: 30}`, `hydra_stop_shadow.enabled: true`. (f) Tests: 138 passing (was 131); added `TestStrikeAdjusterLive`, `TestBreachExitLive`, `TestHydraShadowStop`.

- **1.27.0** (2026-05-04): Brandon Trojan Horse stack — new variants B/C replace the retired pivot experiments (old B = `stressed_only`, old C = `both_sides`, both at 75pt). New variants share three Brandon additions on top of HYDRA's MEIC core: (a) **Take-profit at 80% credit captured** (LIVE in B/C) — closes the IC when its mark decays to ≤20% of credit received via the existing `_close_entry_early` machinery; threshold configurable via `strategy.brandon.take_profit.threshold`. (b) **GEX strike adjuster** (SHADOW in B/C for the first 4 weeks) — fetches Polygon SPX option chain, computes per-strike Gamma Exposure (Σ OI×γ×S²×100, dealers-short-calls / long-puts convention), and per side returns KEEP / SHIFT / SKIP based on detected positive-gamma deceleration walls and negative-gamma acceleration zones. Falls back to BS-gamma from IV when Polygon's `greeks` field is missing (Options Starter tier). (c) **GEX breach exit** (SHADOW) — sustained-90s breach of the outermost decel wall on the threatened side logs a "would-close" event for shadow comparison against actual stops; promotable to live by flipping `strategy.brandon.gex.breach_exit_mode` to `live`. Plus a **defensive overlay proposer** (SHADOW) that returns a debit-spread (before 12:30 ET) or a butterfly pinning a decel-wall midpoint (from 12:30 ET) when SPX comes within `trigger_distance_pts` (default 25pt) of a short strike + GEX confirms an accel zone on that side. **Spread width**: variant B uses HYDRA's MKT-027 dynamic widths (75pt cap, mirrors A); variant C uses Brandon's narrow rule (5pt at VIX<22, 10pt otherwise) via the new `narrow_spread.narrow_spread_width()` function which overrides `_get_vix_adjusted_spread_width` when `strategy.brandon.narrow_spread.enabled: true`. Implementation lives in `bots/hydra/brandon/` as a `BrandonHydraStrategy(HydraStrategy)` subclass selected by `bots/hydra/main.py:230` when `strategy.brandon.enabled: true`. **Variant A's path is fully untouched** — `strategy.py` has zero references to Brandon code; A still loads `HydraStrategy` directly. **Polygon dependency**: set `POLYGON_API_KEY` env var on the VM (or systemd `Environment=`); absent key disables GEX features gracefully (no crashes, GEX-shadow logging silently skipped). **`directional_pivot` is disabled** across all variants in v1.27 (Brandon's GEX-breach exit plays the same protective role); pivot logic is preserved as a togglable alternative — flip `directional_pivot.enabled: true` per-variant to revive. **All three variants run dry-run** on the VM; promoting B/C to live is one config flip per file (`dry_run: false`). **Tests**: 131 new + 79 pre-existing = 210 passing. New files: `bots/hydra/brandon/{__init__,strategy,take_profit,gex_provider,gex_strike_adjuster,gex_breach_exit,defensive_overlay,narrow_spread}.py`, `bots/hydra/config/config_variant_{b,c}.json`, 7 new test files under `tests/test_brandon_*.py`. Service descriptions in `deploy/hydra_variant_{b,c}.service` updated to reflect new purposes.

- **1.26.0** (2026-05-01): Directional pivot strategy + spx_open 9:30 anchor + 75pt × 2c baseline. Three additive features: (a) `MarketData.update_spx` and `update_vix` now gate intraday-OHLC capture (spx_open / spx_high / spx_low / vix_open / vix_high / vix_low) to >= 9:30 ET via new `_is_regular_session_or_later` static method — pre-market Saxo extended-hours quotes (available from 7:00 AM ET) no longer pollute the "% from open" reference used by Upday-035, Downday-035, whipsaw filter, ROC gate, and the new pivot strategy. (b) New `_restore_market_ohlc_from_state_file_unconditional()` called once at end of HYDRA `__init__` so mid-day restarts in dry-run preserve the actual 9:30 anchor (the standard `_recover_positions_from_saxo` path short-circuits in dry mode and never reached `_load_state_file_history`'s OHLC restore). (c) Directional pivot strategy gated by `directional_pivot.enabled` (variant B/C only — A is the control): pre-entry defer-and-watch gate (skip + watch up to `pre_entry_defer_minutes`, default 15) when SPX is already breached at entry time + continuous breach monitor (any-time SPX ±0.25% from open closes all open base entries via configured `close_mode`: `stressed_only` for variant B = close call side on up-breach / put side on down-breach, `both_sides` for variant C = close all 4 legs). Conditional E#3 at 14:00 (Upday-035 / Downday-035) is unaffected. Idempotent. New `IronCondorEntry.{call,put}_side_pivot_closed` flags and 4 new `MEICDailyState` fields (directional_pivot_fired, directional_pivot_direction, directional_pivot_fired_at, deferred_entries) persisted across restarts. Stop-loss vs pivot precedence: stops take precedence (Option 1) — already-stopped sides are skipped on subsequent pivot fire. New main-loop hook in `bots/hydra/main.py` calls `_check_directional_pivot_continuous` BEFORE the strategy state machine, wrapped in try/except. New Telegram alerts: LOW-priority "Entry #N Deferred (directional bias)" + MEDIUM-priority "Directional Pivot Fired (UP/DOWN-breach)". Dashboard variant labels updated: A "(75pt baseline)", B "(pivot, stressed-only)", C "(pivot, both-legs)". VM configs: all 3 variants now 75pt × 2c; B/C `entry_times` = ["10:15","10:45"] (re-enabled 10:15) + `directional_pivot` block enabled with their respective close_mode; A unchanged (control with current entry times + VIX-regime drop of 10:15).
- **1.25.1** (2026-05-01): N-way variant comparison + variant C scaffolding. Refactored backend `dashboard/backend/routers/variants.py` (registry pattern instead of hardcoded a/b ternaries), backend `dashboard/backend/config.py` (variant_c_* paths/labels), frontend `dashboard/frontend/src/pages/Comparison.tsx` (renders N variants from `/api/variants/health`, 3+ variant grouped-bar daily chart). Bot strategy now auto-discovers variants via filesystem glob (`_discover_variant_ids`); Telegram `/compare` and end-of-day `VARIANT_COMPARISON_DAILY` alert render all running non-A variants with per-variant Δ-vs-A lines. New `api_pacing_multiplier` config (default 1.0 = variant A no-op) scales monitoring loop + heartbeat intervals: B=1.5×, C=2.0× to keep combined Saxo API rate under ~60 req/min sustained limit. Vigilant mode (stop detection) is intentionally NOT scaled — safety-critical 2s detection preserved on every variant. New `deploy/hydra_variant_c.service` mirrors variant B's unit with `HYDRA_VARIANT_ID=c`. Variant C config: `max_spread_width: 25` (vs A=50, B=110).
