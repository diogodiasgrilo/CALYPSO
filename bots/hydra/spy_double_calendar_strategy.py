...
class SpyDoubleCalendarStrategy(CalendarStrategyBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not kwargs.get("dry_run", False):
            raise ConfigError(
                ...
                "implemented (simulated from real mids; no real orders), but there is "
                "NO REAL-ORDER EXECUTION PATH AT ALL — this module and "
                "calendar_strategy_base contain ZERO place_order / "
                "place_and_wait_for_fill calls. That, plus SPY-specific American "
                "early-assignment + dividend handling, is the actual blocker. "
                "CORRECTED 2026-09-06: this message previously named the coexistence "
                "MUST-FIXes (STATE-004 / orphan-sweep scoping, buying-power budget) as "
                "the requirement. They are real GO-LIVE GATES but they are NOT what "
                "blocks E today and must not be built first — a halt guarding against "
                "unexpected OPEN positions cannot bind on a strategy that cannot open "
                "one. Going live is a gated build, NOT a config flip. "
    ...
    def _initiate_entry(self) -> str:
        ...
        try:
            ...
            if not self._dc_simulate_entry(entry):
                ...
                # never happened once in E's history, but was previously invisible
                # even to a manual SQL query if it did). send_alert=False: E's
                # config _comment_alerts explicitly documents "E's dry-run path
                # emits none" as an intentional invariant (avoids paging on a
                # scaffold/dry-run variant) — this fix must not silently break
                # that. A daily-summary alert still surfaces the count via
                # entries_failed (shared/alert_service.py daily_summary_ic). NOTE:
                # this does NOT appear on E's dedicated /dc calendar dashboard
                # (which reads dc_calendar.db, a separate recorder) — only in
                # skipped_entries. Full /dc-view parity is a separate follow-up.
    ...
    def _dc_settle_entry(self, entry, spx: float) -> None:
        ...
        # Stamp the close-side commission so this leftover-close backstop records
        # it in dc_outcomes (and the edge reader charges it) like the profit-ladder
        # close — instead of $0. Mirrors _dc_close_calendar (4 legs). (2026-06-23 audit.)
