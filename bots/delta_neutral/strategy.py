...
class DeltaNeutralStrategy:
    ...
    def start_new_trading_day(self):
        ...
        # MKT-001: Check for overnight/weekend gaps at market open
        self.check_pre_market_gap()

    def check_pre_market_gap(self) -> tuple:
        """
        MKT-001: Check if SPY gapped significantly from previous close.

        A large gap (>= 2%) at open indicates high volatility or overnight news event.
        This is logged for awareness - Delta Neutral doesn't block entry but the trader
        should be aware of unusual market conditions.

        This is called at market open each day to log overnight/weekend gaps.

        Returns:
            Tuple[bool, float, str]: (is_significant_gap, gap_percent, message)
        """
        gap_threshold_percent = self.strategy_config.get("premarket_gap_threshold_percent", 2.0)

        # Get previous close from stored daily tracking
        prev_close = self.metrics.spy_open if hasattr(self.metrics, 'spy_open') and self.metrics.spy_open > 0 else 0.0
        ...
        if prev_close == 0:
            logger.info("MKT-001: No previous close available - skipping gap check (first day of trading)")
            return False, 0.0, "No previous close data"
        ...
        if current_price == 0:
            ...
            return False, 0.0, "No current price data"
        ...
        if gap_abs >= gap_threshold_percent:
            ...
            message = (
                f"MKT-001: Large {gap_type} gap detected! SPY gapped {direction} {gap_abs:.2f}% "
            ...
            if self.trade_logger:
                self.trade_logger.log_safety_event({
                    "event_type": "DN_LARGE_GAP_DETECTED",
                    "severity": "WARNING",
                    "description": f"{gap_type.capitalize()} gap {direction} {gap_abs:.2f}%",
                    "action_taken": "LOGGED_WARNING",
                    "result": "Trader should review market conditions",
                    "spy_price": current_price,
                    "prev_close": prev_close,
                    "gap_percent": gap_percent,
                })

            return True, gap_percent, message
        else:
            # Log normal gap for awareness
            direction = "up" if gap_percent > 0 else "down"
            logger.info(
                f"MKT-001: {gap_type.capitalize()} gap check - SPY {direction} {gap_abs:.2f}% "
    ...
    def get_premarket_analysis(self, current_spy_price: float = 0.0) -> dict:
        ...
        result["current_price"] = current_spy_price

        # Calculate gap
        gap_points = current_spy_price - prev_close
        ...
        # Check if shorts might go ITM
        if self.short_strangle:
            short_call_strike = self.short_strangle.call_strike if self.short_strangle.call else 0
            short_put_strike = self.short_strangle.put_strike if self.short_strangle.put else 0

            if short_call_strike > 0 and current_spy_price >= short_call_strike:
                impacts.append(f"⚠️ SHORT CALL ITM: SPY ${current_spy_price:.2f} >= Call strike ${short_call_strike}")
            elif short_call_strike > 0:
                buffer_to_call = short_call_strike - current_spy_price
                if buffer_to_call <= 2:
                    impacts.append(f"SHORT CALL CLOSE: Only ${buffer_to_call:.2f} below Call strike ${short_call_strike}")

            if short_put_strike > 0 and current_spy_price <= short_put_strike:
                impacts.append(f"⚠️ SHORT PUT ITM: SPY ${current_spy_price:.2f} <= Put strike ${short_put_strike}")
            elif short_put_strike > 0:
                buffer_to_put = current_spy_price - short_put_strike
                if buffer_to_put <= 2:
        ...
        if self.long_straddle and self.long_straddle.entry_strike:
            ...
            if move_from_strike >= self.recenter_threshold:
                direction_word = "above" if current_spy_price > long_strike else "below"
                impacts.append(
                    f"⚠️ RECENTER TRIGGERED: SPY ${current_spy_price:.2f} is ${move_from_strike:.1f} pts "
                    f"{direction_word} long strike ${long_strike} (threshold: {self.recenter_threshold} pts)"
                )
            elif move_from_strike >= self.recenter_threshold * 0.8:  # Within 80% of threshold
                impacts.append(
                    f"RECENTER NEAR: SPY ${move_from_strike:.1f} pts from long strike ${long_strike} "
        ...
        if self.long_straddle and self.long_straddle.entry_strike:
            ...
            if move_from_strike >= emergency_threshold:
                impacts.append(
                    f"🚨 EMERGENCY EXIT: Move ${move_from_strike:.1f} exceeds {emergency_pct}% threshold!"
                )
        ...
        # Build message
        if result["is_monday"]:
            msg = f"{gap_type} GAP: SPY {direction} ${abs(gap_points):.2f} ({gap_abs:.2f}%)"
        else:
            msg = f"{gap_type} gap: SPY {direction} ${abs(gap_points):.2f} ({gap_abs:.2f}%)"

        msg += f" | Prev close: ${prev_close:.2f} → Now: ${current_spy_price:.2f}"
        result["message"] = msg
