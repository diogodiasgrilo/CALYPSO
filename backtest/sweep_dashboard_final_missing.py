"""
HYDRA Final Missing Sweeps — 1-Minute Resolution

Sweeps upday_threshold_pct (E6 is now ON) and spread width floors/cap.

Run: python -m backtest.sweep_dashboard_final_missing
"""
import csv
...
SWEEPS = {
    "upday_threshold_pct": {
        ...
        "values": [v / 100 for v in range(20, 105, 5)],  # 0.20%–1.00% in 0.05%
        "label_fmt": "{:.2f}%",
        "live_value": 0.60,
        "description": "E6 upday put-only trigger threshold",
    },
    "call_min_spread": {
        "param": "call_min_spread_width",
        "values": [15, 20, 25, 30, 35, 40, 50],
        "label_fmt": "{}pt",
        "live_value": 25,
        "description": "Min call spread width floor",
    },
    "put_min_spread": {
        "param": "put_min_spread_width",
        "values": [15, 20, 25, 30, 35, 40, 50],
        "label_fmt": "{}pt",
        "live_value": 25,
        "description": "Min put spread width floor",
    },
    "max_spread_width": {
        "param": "max_spread_width",
        "values": [75, 85, 100, 115, 125, 150],
        "label_fmt": "{}pt",
        "live_value": 100,
        "description": "Max spread width cap",
    },
}
...
def main():
    ...
    for name, cfg in SWEEPS.items():
        ...
        live = cfg["label_fmt"].format(cfg["live_value"])
        sweep_lines.append(f"  [cyan]{name:<22}[/] {n:>3} configs  {lo}–{hi}  [dim](current: {live})[/]")

    header = (
        f"[bold cyan]HYDRA Final Missing Sweeps[/]\n"
    ...
    console.print()
    console.print(Panel(header, box=box.ROUNDED, border_style="green",
                        title="[bold white]⚡ HYDRA Final Sweeps[/]"))
    console.print()
    ...
    # Summary
    summary_tbl = Table(
        title="[bold]Final Missing Sweeps Summary[/]",
    ...
    csv_path = out_dir / f"sweep_final_missing_1min_{ts}.csv"
    if all_results:
