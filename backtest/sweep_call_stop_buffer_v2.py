"""
Call Stop Buffer Sensitivity Sweep: Test if adjusting call stop buffer improves Sharpe.

SCOPE: Tests call_stop_buffer variations with all 42 credit gate combinations.
- call_stop_buffer: [50, 75, 100, 125] points ($0.50, $0.75, $1.00, $1.25)
- Credit gates: all 42 combos (7 call × 6 put values)
- Total: 168 backtests (~10 minutes)

Answers: Does call stop buffer width affect optimal credit gates or overall Sharpe?
- Current: call_stop_buffer=$0.75pt achieves Sharpe 0.972
- Question: Can tighter/wider call buffers ($0.50, $1.00, $1.25) exceed 0.972 or are they suboptimal?

Run: python -u -m backtest.sweep_call_stop_buffer_v2 --workers 10
"""
import argparse
import csv
...
BASE_BASELINE = {
    ...
    "call_stop_buffer": 75.0,  # $0.75 (will be overridden)
...
CALL_STOP_BUFFERS = [50, 75, 100, 125]

OUT_DIR = Path("backtest/results")
RESULTS_CSV = OUT_DIR / "call_stop_buffer_results.csv"
PROGRESS_FILE = OUT_DIR / "call_stop_buffer_progress.txt"

CSV_FIELDS = [
    "call_stop_buffer", "combo_id",
...
def _build_cfg(baseline: dict, combo: dict, call_stop_buf: int) -> BacktestConfig:
    cfg = live_config()
    ...
    # Apply call_stop_buffer override
    cfg.call_stop_buffer = call_stop_buf
...
def _worker(args):
    ...
    call_stop_buf, combo_id, combo = args
    t0 = time.time()

    try:
        cfg = _build_cfg(BASE_BASELINE, combo, call_stop_buf)
        ...
        return {
            "call_stop_buffer": call_stop_buf,
            "combo_id": combo_id,
            "sharpe": sharpe,
            "total_pnl": total_net,
            "max_dd": max_dd,
...
def _write_progress(overall_pct, call_buf, call_pct, completed_overall, total_overall, best_overall):
    """Write progress file."""
    bar_width = 40
    overall_bar = "█" * int(bar_width * overall_pct) + "░" * (bar_width - int(bar_width * overall_pct))
    call_bar = "█" * int(bar_width * call_pct) + "░" * (bar_width - int(bar_width * call_pct))

    lines = [
        f"CALL STOP BUFFER SENSITIVITY SWEEP",
        f"",
        f"  Overall [{overall_bar}] {overall_pct*100:.1f}%",
        f"  Completed: {completed_overall} / {total_overall}",
        f"",
        f"  Current param: call_stop_buffer = ${call_buf/100:.2f}",
        f"  [{call_bar}] {call_pct*100:.1f}%",
        f"",
        f"  GLOBAL BEST FOUND:",
        f"    {best_overall['status']}",
        f"    call_stop_buffer=${best_overall['call_buf']:.2f}, call=${best_overall['call']:.2f}, put=${best_overall['put']:.2f}",
...
if __name__ == "__main__":
    ...
    total_backtests = len(CALL_STOP_BUFFERS) * combo_count
    ...
    print(f"\n{'='*90}")
    print(f"📊 CALL STOP BUFFER SENSITIVITY SWEEP")
    print(f"{'='*90}")
    print(f"Call stop buffers: {[f'${b/100:.2f}' for b in CALL_STOP_BUFFERS]}")
    ...
    print(f"Baseline (locked): put_stop_buffer=${BASE_BASELINE['put_stop_buffer']/100:.2f}, buffer_decay={BASE_BASELINE['buffer_decay_start_mult']}×/{BASE_BASELINE['buffer_decay_hours']:.1f}h")
    print(f"{'='*90}\n")
    ...
    for buf_idx, call_stop_buf in enumerate(CALL_STOP_BUFFERS):
        print(f"\n[{buf_idx+1}/{len(CALL_STOP_BUFFERS)}] CALL_STOP_BUFFER = ${call_stop_buf/100:.2f}")
        print(f"{'='*90}\n")

        buf_best_sharpe = -999
        buf_best_result = None
        worker_args = [(call_stop_buf, i+1, combo) for i, combo in enumerate(combos)]
        completed_combo = 0
        t_start = time.time()
            ...
            for result in pool.imap_unordered(_worker, worker_args, chunksize=1):
                if result:
                    ...
                    with open(RESULTS_CSV, "a", newline="") as f:
                        writer = csv.DictWriter(f, fieldnames=CSV_FIELDS)
                        writer.writerow({
                            "call_stop_buffer": f'${result["call_stop_buffer"]/100:.2f}',
                    ...
                    if result["sharpe"] > buf_best_sharpe:
                        buf_best_sharpe = result["sharpe"]
                        buf_best_result = result
                    ...
                    _write_progress(overall_pct, call_stop_buf, buf_pct, completed_overall, total_backtests, best_info)
        ...
        if buf_best_result:
            diff = buf_best_sharpe - CURRENT_BEST
            status = "🟢 BETTER" if diff > 0.01 else ("🔴 WORSE" if diff < -0.01 else "🟡 SIMILAR")
            print(f"\n  {status}: Sharpe {buf_best_sharpe:.3f} (Δ{diff:+.3f}), P&L ${buf_best_result['total_pnl']:+,.0f}\n")
    ...
    if global_best_result:
        ...
        print(f"{verdict}: Sharpe {global_best_sharpe:.3f} (call_buf=${global_best_result['call_stop_buffer']/100:.2f})")
    print(f"{'='*90}\n")
