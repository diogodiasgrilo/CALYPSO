"""
Put-Only Max VIX Sweep (NEW COMBO): Test VIX gate for put-only entries with improved credit combo.

SCOPE: Tests put_only_max_vix variations with the NEW credit combo (call=$2.00, put=$2.75).
- put_only_max_vix: [10.0, 15.0, 18.0, 20.0, 25.0, 30.0] (max VIX to allow put-only entries)
- Credit gates: FIXED at min_call_credit=2.00, min_put_credit=2.75 (showed +0.047 Sharpe improvement)
- Total: 6 backtests (~4 minutes)

Background: When the call spread credit is non-viable (< min_call_credit), HYDRA can place put-only entries.
But it only does so if VIX < put_only_max_vix (safety gate to avoid puts in high-volatility environment).

Hypothesis: With the TIGHTER credit floors in the new combo, put-only entries become more viable.
We can likely allow higher VIX to place put-only entries without increasing risk.

This is a NEW parameter interaction test - the new combo's tight credit floors fundamentally
change the risk/reward of one-sided entries, so the optimal VIX gate should shift.

Run: python -u -m backtest.sweep_put_only_max_vix_new_combo --workers 2
"""
import argparse
import csv
...
BASE_BASELINE = {
    ...
    "put_only_max_vix": 15.0,  # Will be overridden by sweep
...
# Put-only max VIX values to test
PUT_ONLY_MAX_VIX = [10.0, 15.0, 18.0, 20.0, 25.0, 30.0]

OUT_DIR = Path("backtest/results")
RESULTS_CSV = OUT_DIR / "put_only_max_vix_new_combo_results.csv"
PROGRESS_FILE = OUT_DIR / "put_only_max_vix_new_combo_progress.txt"
...
def _build_cfg(baseline: dict, put_only_max_vix: float) -> BacktestConfig:
    cfg = live_config()
    ...
    cfg.put_only_max_vix = put_only_max_vix
...
def _worker(args):
    ...
    idx, put_only_max_vix = args
    t0 = time.time()

    try:
        cfg = _build_cfg(BASE_BASELINE, put_only_max_vix)
        ...
        return {
            "put_only_max_vix": put_only_max_vix,
            "sharpe": sharpe,
            "total_pnl": total_net,
            "max_dd": max_dd,
            "calmar": calmar,
...
def _write_progress(pct, completed, total, best_result):
    ...
    lines = [
        f"PUT-ONLY MAX VIX SWEEP (NEW COMBO: call=$2.00, put=$2.75)",
        f"",
        f"  [{bar}] {pct*100:.1f}%",
    ...
    if best_result:
        ...
        lines.extend([
            f"  BEST FOUND: {status}",
            f"    put_only_max_vix={best_result['put_only_max_vix']:.1f}, Sharpe={best_result['sharpe']:.3f} (Δ{delta:+.3f})",
...
if __name__ == "__main__":
    ...
    total_backtests = len(PUT_ONLY_MAX_VIX)
    ...
    print(f"\n{'='*90}")
    print(f"📊 PUT-ONLY MAX VIX SWEEP (NEW COMBO: call=$2.00, put=$2.75)")
    print(f"{'='*90}")
    print(f"Put-only max VIX values: {PUT_ONLY_MAX_VIX}")
    print(f"Total backtests: {total_backtests}")
    print(f"Workers: {args.workers}")
    print(f"Period: {FULL_START} to {FULL_END}")
    print(f"Baseline (locked): put_buffer=${BASE_BASELINE['put_stop_buffer']/100:.2f}, call_buffer=${BASE_BASELINE['call_stop_buffer']/100:.2f}, decay_mult={BASE_BASELINE['buffer_decay_start_mult']:.1f}×, decay_hours={BASE_BASELINE['buffer_decay_hours']:.1f}h")
    print(f"Current best: Sharpe {CURRENT_BEST}")
    print(f"Hypothesis: Tighter credit floors in new combo → can allow higher VIX for put-only entries")
    print(f"{'='*90}\n")
    ...
    worker_args = list(enumerate(PUT_ONLY_MAX_VIX))
        ...
        for result in pool.imap_unordered(_worker, worker_args, chunksize=1):
            if result:
                ...
                with open(RESULTS_CSV, "a", newline="") as f:
                    writer = csv.DictWriter(f, fieldnames=CSV_FIELDS)
                    writer.writerow({
                        "put_only_max_vix": f'{result["put_only_max_vix"]:.1f}',
                ...
                bar_width = 35
                bar = "█" * int(bar_width * pct) + "░" * (bar_width - int(bar_width * pct))
                print(f"  [{bar}] {pct*100:.1f}% | put_only_max_vix={result['put_only_max_vix']:.1f} Sharpe {result['sharpe']:.3f} | ETA {remaining/60:.1f}min")
                sys.stdout.flush()

    print(f"\n{'='*90}")
    print(f"✅ PUT-ONLY MAX VIX SWEEP COMPLETE")
    ...
    if global_best_result:
        ...
        print(f"{verdict}: Sharpe {global_best_sharpe:.3f} (put_only_max_vix={global_best_result['put_only_max_vix']:.1f})")
