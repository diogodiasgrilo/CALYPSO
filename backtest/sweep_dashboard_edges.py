"""
HYDRA Edge Extension Sweep — 1-Minute Resolution

Extends ranges for parameters that hit sweep boundaries.

Run: python -m backtest.sweep_dashboard_edges
"""
import csv
...
SWEEPS = {
    "call_credit_floor": {
        ...
        "values": [v / 100 for v in range(50, 95, 5)],  # $0.50–$0.90 in $0.05
        "label_fmt": "${:.2f}",
        "live_value": 0.85,
        "description": "MKT-029 call fallback floor (edge extension ↓)",
    },
    "downday_theo_put": {
        "param": "downday_theoretical_put_credit",
        "values": [float(v) for v in range(280, 360, 10)],  # $2.80–$3.50 in $0.10
        "label_fmt": "${:.2f}",
        "label_div": 100,
        "live_value": 290.0,
        "description": "Theo put for call-only stops (edge extension ↑)",
    },
}
...
def main():
    ...
    header = (
        f"[bold cyan]HYDRA Edge Extension Sweep[/]\n"
    ...
    console.print()
    console.print(Panel(header, box=box.ROUNDED, border_style="yellow",
                        title="[bold white]⚡ HYDRA Edge Check[/]"))
    console.print()
    ...
    summary_tbl.add_column("Verdict", width=20)
    ...
    for sname, cfg in SWEEPS.items():
        ...
        at_edge = (best["value"] == cfg["values"][0] or best["value"] == cfg["values"][-1])
        if at_edge:
            verdict = "[bold red]STILL AT EDGE[/]"
        elif abs(best["value"] - live_val) < 0.001:
            verdict = "[bold green]✓ CONFIRMED[/]"
        else:
            verdict = "[bold yellow]SHIFT[/]"
        ...
        summary_tbl.add_row(
            ...
            f"[{delta_style}]{delta:+.3f}[/{delta_style}]", verdict,
        )
    ...
    csv_path = out_dir / f"sweep_edges_1min_{ts}.csv"
    if all_results:
