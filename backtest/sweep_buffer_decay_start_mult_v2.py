"""
Buffer Decay Start Multiplier Sweep: Test if different initial buffer multipliers improve Sharpe.

SCOPE: Tests buffer_decay_start_mult variations with all 42 credit gate combinations.
- buffer_decay_start_mult: [1.5, 2.0, 2.5, 3.0]
- Credit gates: all 42 combos (7 call × 6 put values)
- Total: 168 backtests (~10 minutes)

Concept: Stop buffers start wider at entry, linearly decay to normal over N hours.
Example: With start_mult=2.5 and normal buffer=$1.75, initial stop buffer=$4.375, decays to $1.75 over 4h.

Answers: Does initial buffer multiplier affect optimal credit gates or overall Sharpe?
- Current: buffer_decay_start_mult=2.5 achieves Sharpe 0.972
- Question: Can different multipliers (1.5, 2.0, 3.0) exceed 0.972?

Run: python -u -m backtest.sweep_buffer_decay_start_mult_v2 --workers 10
"""
import argparse
import csv
...
BASE_BASELINE = {
    ...
    "buffer_decay_start_mult": 2.5,  # Will be overridden
...
# Buffer decay start multiplier values to test
DECAY_START_MULTS = [1.5, 2.0, 2.5, 3.0]

OUT_DIR = Path("backtest/results")
RESULTS_CSV = OUT_DIR / "buffer_decay_start_mult_results.csv"
PROGRESS_FILE = OUT_DIR / "buffer_decay_start_mult_progress.txt"

CSV_FIELDS = [
    "decay_start_mult", "combo_id",
...
def _build_cfg(baseline: dict, combo: dict, decay_mult: float) -> BacktestConfig:
    cfg = live_config()
    ...
    # Apply buffer_decay_start_mult override
    cfg.buffer_decay_start_mult = decay_mult
...
def _worker(args):
    ...
    decay_mult, combo_id, combo = args
    t0 = time.time()

    try:
        cfg = _build_cfg(BASE_BASELINE, combo, decay_mult)
...
def _write_progress(overall_pct, decay_mult, decay_pct, completed_overall, total_overall, best_overall):
    """Write progress file."""
    bar_width = 40
    ...
    lines = [
        f"BUFFER DECAY START MULTIPLIER SWEEP",
        f"",
        f"  Overall [{overall_bar}] {overall_pct*100:.1f}%",
        f"  Completed: {completed_overall} / {total_overall}",
        f"",
        f"  Current param: buffer_decay_start_mult = {decay_mult:.1f}×",
        f"  [{decay_bar}] {decay_pct*100:.1f}%",
        f"",
        f"  GLOBAL BEST FOUND:",
        f"    {best_overall['status']}",
        f"    decay_start_mult={best_overall['decay_mult']:.1f}×, call=${best_overall['call']:.2f}, put=${best_overall['put']:.2f}",
...
if __name__ == "__main__":
    ...
    total_backtests = len(DECAY_START_MULTS) * combo_count
    ...
    print(f"\n{'='*90}")
    print(f"📊 BUFFER DECAY START MULTIPLIER SWEEP")
    print(f"{'='*90}")
    print(f"Decay start multipliers: {DECAY_START_MULTS}")
    ...
    print(f"Baseline (locked): put_buffer=${BASE_BASELINE['put_stop_buffer']/100:.2f}, call_buffer=${BASE_BASELINE['call_stop_buffer']/100:.2f}, decay_hours={BASE_BASELINE['buffer_decay_hours']:.1f}h")
    print(f"{'='*90}\n")
    ...
    for mult_idx, decay_mult in enumerate(DECAY_START_MULTS):
        print(f"\n[{mult_idx+1}/{len(DECAY_START_MULTS)}] DECAY_START_MULT = {decay_mult:.1f}×")
        print(f"{'='*90}\n")

        mult_best_sharpe = -999
        mult_best_result = None
        worker_args = [(decay_mult, i+1, combo) for i, combo in enumerate(combos)]
        completed_combo = 0
        t_start = time.time()
            ...
            for result in pool.imap_unordered(_worker, worker_args, chunksize=1):
                if result:
                    ...
                    with open(RESULTS_CSV, "a", newline="") as f:
                        writer = csv.DictWriter(f, fieldnames=CSV_FIELDS)
                        writer.writerow({
                            "decay_start_mult": f'{result["decay_start_mult"]:.1f}',
                    ...
                    if result["sharpe"] > mult_best_sharpe:
                        mult_best_sharpe = result["sharpe"]
                        mult_best_result = result
                    ...
                    _write_progress(overall_pct, decay_mult, mult_pct, completed_overall, total_backtests, best_info)
        ...
        if mult_best_result:
            diff = mult_best_sharpe - CURRENT_BEST
            status = "🟢 BETTER" if diff > 0.01 else ("🔴 WORSE" if diff < -0.01 else "🟡 SIMILAR")
            print(f"\n  {status}: Sharpe {mult_best_sharpe:.3f} (Δ{diff:+.3f}), P&L ${mult_best_result['total_pnl']:+,.0f}\n")

    print(f"\n{'='*90}")
    print(f"✅ BUFFER DECAY START MULTIPLIER SWEEP COMPLETE")
    ...
    if global_best_result:
        ...
        print(f"{verdict}: Sharpe {global_best_sharpe:.3f} (decay_mult={global_best_result['decay_start_mult']:.1f}×)")
    print(f"{'='*90}\n")
