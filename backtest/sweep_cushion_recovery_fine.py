"""
Fine-grain sweep of cushion recovery exit around Near 93-97% × Recv 65-80%.

Prior sweep found Near 95% Recv 70% beats baseline on Sharpe (2.152 vs 2.094).

Run: python -m backtest.sweep_cushion_recovery_fine
"""
...
def main():
    ...
    # Fine-grain: Near 91-98% × Recv 60-85% in 1-2% steps
    for near in [0.91, 0.92, 0.93, 0.94, 0.95, 0.96, 0.97, 0.98]:
        for recv in [0.60, 0.65, 0.68, 0.70, 0.72, 0.75, 0.78, 0.80, 0.85]:
            if recv >= near:
                continue
            cfg = _base()
    ...
    hdr = (f"{'Strategy':<14s}  {'P&L':>10s}  {'Δ P&L':>8s}  {'Sharpe':>7s}  {'MaxDD':>8s}  "
    ...
    with mp.Pool(N_WORKERS, maxtasksperchild=4) as pool:
        for res in pool.imap_unordered(_run_one, tasks):
            ...
            print(f"  [{idx+1:2d}/{n}] {label:<12s}  ${m['total_pnl']:>+9,.0f}  {'':>8s}  {m['sharpe']:>7.3f}  "
    ...
    print()
    print("=" * 100)
    print("RESULTS SORTED BY SHARPE RATIO (top 25)")
    print("=" * 100)
    print(f"{'Strategy':<14s}  {'P&L':>10s}  {'Δ P&L':>8s}  {'Sharpe':>7s}  {'MaxDD':>8s}  "
          f"{'Win%':>5s}  {'AvgWin':>7s}  {'AvgLoss':>8s}  {'Exits':>5s}")
    print("─" * 100)

    for _, label, m in results[:25]:
