"""
Buffer Decay Start Multiplier Sweep (NEW COMBO): Re-test with improved credit gate combo.

SCOPE: Tests buffer_decay_start_mult variations with the NEW credit combo (call=$2.00, put=$2.75).
- buffer_decay_start_mult: [1.5, 2.0, 2.5, 3.0, 3.5] (initial width of decay buffer)
- Credit gates: FIXED at min_call_credit=2.00, min_put_credit=2.75 (showed +0.047 Sharpe improvement)
- Total: 5 backtests (~3 minutes)

Background: Buffer decay formula: buffer(t) = normal + (normal * (mult - 1)) * max(0, 1 - elapsed_hours / decay_hours)
- mult=1.0: no decay (buffer fixed at normal value)
- mult=2.0: buffer starts at 2× normal, decays to 1× normal over decay_hours
- mult=3.0: buffer starts at 3× normal, decays to 1× normal over decay_hours

The put_stop_buffer sweep found that (call=$2.00, put=$2.75) achieves Sharpe 0.972 vs baseline 0.925.
This sweep tests if decay start multipliers also improve with the new combo.

Run: python -u -m backtest.sweep_buffer_decay_start_mult_new_combo --workers 2
"""
import argparse
import csv
...
BASE_BASELINE = {
    ...
    "buffer_decay_start_mult": 2.5,  # Will be overridden by sweep
...
# Decay start multiplier values to test
DECAY_START_MULTS = [1.5, 2.0, 2.5, 3.0, 3.5]

OUT_DIR = Path("backtest/results")
RESULTS_CSV = OUT_DIR / "buffer_decay_start_mult_new_combo_results.csv"
PROGRESS_FILE = OUT_DIR / "buffer_decay_start_mult_new_combo_progress.txt"

CSV_FIELDS = [
    "buffer_decay_start_mult",
...
def _build_cfg(baseline: dict, decay_start_mult: float) -> BacktestConfig:
    cfg = live_config()
    ...
    cfg.buffer_decay_start_mult = decay_start_mult
...
def _worker(args):
    ...
    idx, decay_start_mult = args
    t0 = time.time()

    try:
        cfg = _build_cfg(BASE_BASELINE, decay_start_mult)
        ...
        return {
            "buffer_decay_start_mult": decay_start_mult,
            "sharpe": sharpe,
            "total_pnl": total_net,
            "max_dd": max_dd,
            "calmar": calmar,
...
def _write_progress(pct, completed, total, best_result):
    ...
    lines = [
        f"BUFFER DECAY START MULTIPLIER SWEEP (NEW COMBO: call=$2.00, put=$2.75)",
        f"",
        f"  [{bar}] {pct*100:.1f}%",
    ...
    if best_result:
        ...
        lines.extend([
            f"  BEST FOUND: {status}",
            f"    decay_start_mult={best_result['buffer_decay_start_mult']:.1f}×, Sharpe={best_result['sharpe']:.3f} (Δ{delta:+.3f})",
...
if __name__ == "__main__":
    ...
    total_backtests = len(DECAY_START_MULTS)
    ...
    print(f"\n{'='*90}")
    print(f"📊 BUFFER DECAY START MULTIPLIER SWEEP (NEW COMBO: call=$2.00, put=$2.75)")
    print(f"{'='*90}")
    ...
    worker_args = list(enumerate(DECAY_START_MULTS))
        ...
        for result in pool.imap_unordered(_worker, worker_args, chunksize=1):
            if result:
                ...
                with open(RESULTS_CSV, "a", newline="") as f:
                    writer = csv.DictWriter(f, fieldnames=CSV_FIELDS)
                    writer.writerow({
                        "buffer_decay_start_mult": f'{result["buffer_decay_start_mult"]:.1f}',
                ...
                bar_width = 35
                bar = "█" * int(bar_width * pct) + "░" * (bar_width - int(bar_width * pct))
                print(f"  [{bar}] {pct*100:.1f}% | decay_mult={result['buffer_decay_start_mult']:.1f}× Sharpe {result['sharpe']:.3f} | ETA {remaining/60:.1f}min")
                sys.stdout.flush()
    ...
    if global_best_result:
        ...
        print(f"{verdict}: Sharpe {global_best_sharpe:.3f} (decay_start_mult={global_best_result['buffer_decay_start_mult']:.1f}×)")
