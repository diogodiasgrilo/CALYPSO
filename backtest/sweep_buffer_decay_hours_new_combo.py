"""
Buffer Decay Hours Sweep (NEW COMBO): Re-test with improved credit gate combo.

SCOPE: Tests buffer_decay_hours variations with the NEW credit combo (call=$2.00, put=$2.75).
- buffer_decay_hours: [2.0, 3.0, 4.0, 5.0] (decay duration in hours)
- Credit gates: FIXED at min_call_credit=2.00, min_put_credit=2.75 (showed +0.047 Sharpe improvement)
- Total: 4 backtests (~2 minutes)

Background: Buffer decay formula: buffer(t) = normal + (normal * (mult - 1)) * max(0, 1 - elapsed_hours / decay_hours)
- decay_hours=2.0: buffer decays to normal faster (richer early, normal later)
- decay_hours=4.0: buffer decays slower (richer for longer)
- decay_hours=5.0: buffer decays very slowly (stays rich until mid-afternoon)

The put_stop_buffer sweep found that (call=$2.00, put=$2.75) achieves Sharpe 0.972 vs baseline 0.925.
This sweep tests if decay durations also improve with the new combo.

Run: python -u -m backtest.sweep_buffer_decay_hours_new_combo --workers 2
"""
import argparse
import csv
...
BASE_BASELINE = {
    ...
    "buffer_decay_hours": 4.0,  # Will be overridden by sweep
...
# Decay hours values to test
DECAY_HOURS = [2.0, 3.0, 4.0, 5.0]

OUT_DIR = Path("backtest/results")
RESULTS_CSV = OUT_DIR / "buffer_decay_hours_new_combo_results.csv"
PROGRESS_FILE = OUT_DIR / "buffer_decay_hours_new_combo_progress.txt"
...
def _build_cfg(baseline: dict, decay_hours: float) -> BacktestConfig:
    cfg = live_config()
...
def _worker(args):
    ...
    idx, decay_hours = args
    t0 = time.time()

    try:
        cfg = _build_cfg(BASE_BASELINE, decay_hours)
        ...
        return {
            "buffer_decay_hours": decay_hours,
            "sharpe": sharpe,
            "total_pnl": total_net,
            "max_dd": max_dd,
            "calmar": calmar,
...
def _write_progress(pct, completed, total, best_result):
    ...
    lines = [
        f"BUFFER DECAY HOURS SWEEP (NEW COMBO: call=$2.00, put=$2.75)",
        f"",
        f"  [{bar}] {pct*100:.1f}%",
    ...
    if best_result:
        ...
        lines.extend([
            f"  BEST FOUND: {status}",
            f"    decay_hours={best_result['buffer_decay_hours']:.1f}h, Sharpe={best_result['sharpe']:.3f} (Δ{delta:+.3f})",
...
if __name__ == "__main__":
    ...
    total_backtests = len(DECAY_HOURS)
    ...
    print(f"\n{'='*90}")
    print(f"📊 BUFFER DECAY HOURS SWEEP (NEW COMBO: call=$2.00, put=$2.75)")
    print(f"{'='*90}")
    ...
    worker_args = list(enumerate(DECAY_HOURS))
        ...
        for result in pool.imap_unordered(_worker, worker_args, chunksize=1):
            if result:
                ...
                with open(RESULTS_CSV, "a", newline="") as f:
                    writer = csv.DictWriter(f, fieldnames=CSV_FIELDS)
                    writer.writerow({
                        "buffer_decay_hours": f'{result["buffer_decay_hours"]:.1f}',
                ...
                bar_width = 35
                bar = "█" * int(bar_width * pct) + "░" * (bar_width - int(bar_width * pct))
                print(f"  [{bar}] {pct*100:.1f}% | decay_hours={result['buffer_decay_hours']:.1f}h Sharpe {result['sharpe']:.3f} | ETA {remaining/60:.1f}min")
                sys.stdout.flush()
    ...
    if global_best_result:
        ...
        print(f"{verdict}: Sharpe {global_best_sharpe:.3f} (decay_hours={global_best_result['buffer_decay_hours']:.1f}h)")
